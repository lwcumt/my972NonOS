...
/*-----------------------------------------*/
 UART_BUFFER_T UART_DEV[UART_NUM];
...
static void _uartReceiveChars(INT nNum)
{
    ...
    do {
        ...
        ucChar = inpb(REG_UART0_RBR+uOffset);
				...
				if(nNum==UART3)
				{
			//	ModBus_CommExec(ucChar);
					
					#ifdef MODBUS_972
						ModBus_CommExec(ucChar);
					#endif
					
			//		ModBus_FIFO_Buf[ModBus_FIFO_Cnt++]=ucChar;//转存到FIFO
////////					#ifdef ONLY_FX_PLC
////////						CommExec_FX_Plc(ucChar);
////////					#else
////////						#ifdef MODBUS_NEW
////////						
////////						ModBus_CommExec(ucChar);
