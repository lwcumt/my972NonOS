...
void ReadFiles(void)
{
//FileRead("开机画面.bmp",(unsigned char*)START_PIC_sdram);//读开机画面
FileRead("Power.bmp",(unsigned char*)START_PIC_sdram);//读开机画面

//GLCD_SetWindow_Fill(30,450,600,479,BLACK_COLOR,BLACK_COLOR);//*******************************

FileRead("背景.bmp",(unsigned char*)WOEK_BACK_dis);//读背景
GLCD_SetWindow_Fill(0,0,799,479,BLACK_COLOR,BLACK_COLOR);//*******************************

FileRead("编程.bmp",(unsigned char*)PROGRAM_sdram);//
FileRead("标签.bmp",(unsigned char*)LABELS_sdram);//
FileRead("参数.bmp",(unsigned char*)PARAMETER_sdram);//
FileRead("尺.bmp",(unsigned char*)RULER_sdram);//
FileRead("等分.bmp",(unsigned char*)DIVIDE_sdram);//
FileRead("时钟.bmp",(unsigned char*)WATCH_sdram);//
FileRead("示教.bmp",(unsigned char*)TEACH_sdram);//
FileRead("手动.bmp",(unsigned char*)MANUAL_sdram);//
FileRead("自动.bmp",(unsigned char*)AUTO_sdram);//
FileRead("选择程序.bmp",(unsigned char*)SELECT_PRO_sdram);//
FileRead("自刀.bmp",(unsigned char*)AUTO_KNIFE_sdram);//

FileRead("标签功能.bmp",(unsigned char*)LABEL_FUN_sdram);//
FileRead("程序.bmp",(unsigned char*)PROCESS_sdram);//
FileRead("工具.bmp",(unsigned char*)TOOLS_sdram);//
FileRead("等分功能.bmp",(unsigned char*)DIVIDE_FUN_sdram);//
FileRead("基准功能.bmp",(unsigned char*)RULER_FUN_sdram);//
FileRead("日历.bmp",(unsigned char*)CALENDAR_sdram);//
...
FileRead("F2退出up.bmp",(unsigned char*)F2_OUT_UP_sdram);//
FileRead("F1尺子up.bmp",(unsigned char*)F1_RULER_UP_sdram);//
FileRead("F2工具up.bmp",(unsigned char*)F2_TOOLS_UP_sdram);//
FileRead("完成.bmp",(unsigned char*)FINISHED_sdram);//
//FileRead("示教模式.bmp",(unsigned char*)TEACH_MOD_sdram);//
//FileRead("自刀模式.bmp",(unsigned char*)AUTO_KNIFE_MOD_sdram);//
FileRead("刀上.bmp",(unsigned char*)KNIFE_UP_sdram);//
FileRead("刀下.bmp",(unsigned char*)KNIFE_DOWN_sdram);//
FileRead("压上.bmp",(unsigned char*)PRESS_UP_sdram);//
FileRead("压下.bmp",(unsigned char*)PRESS_DOWN_sdram);//
FileRead("红色A.bmp",(unsigned char*)RED_A_sdram);//
FileRead("红色B.bmp",(unsigned char*)RED_B_sdram);//
FileRead("黑色A.bmp",(unsigned char*)BLACK_A_sdram);//
FileRead("黑色B.bmp",(unsigned char*)BLACK_B_sdram);//
FileRead("红线.bmp",(unsigned char*)RED_LINE_sdram);//
FileRead("手红外.bmp",(unsigned char*)HAND_IR_sdram);//
FileRead("刀前.bmp",(unsigned char*)FRONT_KNIFE_sdram);//
FileRead("红刀.bmp",(unsigned char*)RED_KNIFE_F_sdram);//
FileRead("推纸.bmp",(unsigned char*)PUSH_PAPER_F_sdram);//
FileRead("压纸循环标记.bmp",(unsigned char*)PRE_AUTO_F_sdram);//
FileRead("进度.bmp",(unsigned char*)SCHEDULE_sdram);//
FileRead("进度位置.bmp",(unsigned char*)SCHEDULE_DOT_sdram);//
FileRead("右箭头.bmp",(unsigned char*)RIGHT_ARROW_sdram);//
FileRead("时间条.bmp",(unsigned char*)TIME_COL_sdram);//
FileRead("推纸器.bmp",(unsigned char*)PUSH_PAPER_TOOL_sdram);//
//FileRead("取消up.bmp",(unsigned char*)CANCEL_UP_sdram);//
//FileRead("取消down.bmp",(unsigned char*)CANCEL_DOWN_sdram);//
//FileRead("确定up.bmp",(unsigned char*)ENTER_UP_sdram);//
//FileRead("确定down.bmp",(unsigned char*)ENTER_DOWN_sdram);//
FileRead("停止up.bmp",(unsigned char*)STOP_UP_sdram);//
//FileRead("停止down.bmp",(unsigned char*)STOP_DOWN_sdram);//
FileRead("运行up.bmp",(unsigned char*)RUN_UP_sdram);//
//FileRead("运行down.bmp",(unsigned char*)RUN_DOWN_sdram);//
FileRead("输入尺寸.bmp",(unsigned char*)INPUT_SIZE_sdram);//
FileRead("提示栏.bmp",(unsigned char*)REMIND_sdram);//
FileRead("推纸_刀.bmp",(unsigned char*)PUSH_KNIFE_sdram);//
FileRead("位置.bmp",(unsigned char*)POSITION_sdram);//
FileRead("序号.bmp",(unsigned char*)ORDER_sdram);//
FileRead("中间活页.bmp",(unsigned char*)MIDDLE_PAGE_sdram);//
FileRead("左边活页.bmp",(unsigned char*)LEFT_PAGE_sdram);//
FileRead("删除up.bmp",(unsigned char*)DELETE_UP_sdram);//
FileRead("全删up.bmp",(unsigned char*)DELETE_ALL_UP_sdram);//
FileRead("鸭梨.bmp",(unsigned char*)PEAR_sdram);//
FileRead("F键up.bmp",(unsigned char*)F_KEY_UP_sdram);//
FileRead("后退up.bmp",(unsigned char*)BACK_PAGE_UP_sdram);//
FileRead("前进up.bmp",(unsigned char*)FRONT_PAGE_UP_sdram);//
FileRead("上箭头up.bmp",(unsigned char*)UP_ARROW_UP_sdram);//
FileRead("下箭头up.bmp",(unsigned char*)DOWN_ARROW_UP_sdram);//
FileRead("F3++up.bmp",(unsigned char*)F3_PLUS_UP_sdram);//
FileRead("F3时钟up.bmp",(unsigned char*)F3_CLOCK_UP_sdram);//
FileRead("F4--up.bmp",(unsigned char*)F4_MINUS_UP_sdram);//
//FileRead("F5刀up.bmp",(unsigned char*)F5_CUT_UP_sdram);//
FileRead("脉冲当量.bmp",(unsigned char*)PULSE_DEGREE_sdram);//
FileRead("锁.bmp",(unsigned char*)LOCK_sdram);//
FileRead("锁 钥匙.bmp",(unsigned char*)LOCK_KEY_sdram);//
FileRead("左右箭头.bmp",(unsigned char*)L_R_ARROW_sdram);//
FileRead("F5压纸循环up.bmp",(unsigned char*)F5_PRE_FLG_BUTT_UP_sdram);//
...
FileRead("快进up.bmp",(unsigned char*)FAST_FORWD_UP_sdram);
FileRead("快退up.bmp",(unsigned char*)FAST_BACK_UP_sdram);
}
