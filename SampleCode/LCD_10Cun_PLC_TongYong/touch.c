...
if(temp<4000000)	//4000000误差之内
	...
	if(temp<50000)	//A1误差之内
		{	
			...
			if(temp<2000000)	//A3误差之内	2000000
				{
					if(A5>A5_PER)
						{
						temp=A5-A5_PER;
...
// if(nand_f==1)
// {	
// Printf24("read ok from nand",350,300,0,1,0xf7a2);
// Int32U dd=100000000;
// while(dd--);
// }
return	0; // ok
...
Int8U n;
	...
	FlashChip_Dis;
#else
	pInt8U pSDbuf=(pInt8U)DOWNFILE_SDRAM_ADD;
	FileRead("touchdata.ini",pSDbuf);
	pdata=(pInt8U)&A0;
	for(n=0;n<4;n++)
		{
		*pdata++=*pSDbuf++;
...
**/
	...
	ChangeFileName("touchdata.ini");
	//NandEccWrite_Flag=0;
	File_Create(iFiles.FileName);//建立文件
	// Printf24("save ok to nand",350,300,0,1,0xf7a2);
#endif
}
...
if(Board_TestMod==TEST_TOUCH)//校准处理
	...
	GetTouchKeyCode();// 获得触摸 键值
		...
		#ifdef PLC_3U		
		if((Manual_Knife_Status==1)||(Fast_Knife_Status==1)||(Motor_Knife_Status==1)||(Pressure_Set_Status==1)||(YaZhi_Process_Flag==1)||(Knife_Change_Status==1))
			{
			return;			
			}		
		#endif
			...
			switch(WorkMode)
				...
				case SELF_CUT_MODE:  //自刀主 模式
					if(Touch_Pos.y>F6_BUTTON_Y2)
						...
						else if((Touch_Pos.y>=TEACH_MOD_TOU_Y1)&&(Touch_Pos.y<=TEACH_MOD_TOU_Y2))
							{
							if((Touch_Pos.x>=TEACH_MOD_TOU_X1)&&(Touch_Pos.x<=TEACH_MOD_TOU_X2))
						...
						if((Touch_Pos.x>=F1_BUTTON_X1)&&(Touch_Pos.x<=F1_BUTTON_X2))  //F1,2,3,4子模式
							...
							{
							if((WorkMode==AUTO_MODE)||(WorkMode==TEACH_MODE))//自动,示教 画面下+++++++++++++++++++201903
								{
								...
								if((Touch_Pos.x>=AIR_FLAG_BUTT_X1)&&(Touch_Pos.x<=AIR_FLAG_BUTT_X2))		//	气垫标记处--------1
									...
									while(row_cou<PageKf_Size)	
										{
										if((Touch_Pos.y>(AIR_FLAG_BUTT_Y1+(row_cou*Kf_Line_HIGH)))
										&&(Touch_Pos.y<(AIR_FLAG_BUTT_Y2+((row_cou+1)*Kf_Line_HIGH))))
											{
											Touch_code=	SHIFT_0_KEY_CODE;	//shift+0------ 气垫标记
											FlagTouRow_num=row_cou+1;
											}
									...
									while(row_cou<PageKf_Size)	
										{
										if((Touch_Pos.y>(OTHER_FLAG_BUTT_Y1+(row_cou*Kf_Line_HIGH)))
										&&(Touch_Pos.y<(OTHER_FLAG_BUTT_Y2+((row_cou+1)*Kf_Line_HIGH))))
											{
											Touch_code=	SHIFT_8_KEY_CODE;	//shift+8------其他标记
											FlagTouRow_num=row_cou+1;
											}
				...
				case READ_PRO_MODE:		//查找模式......程序选择
					if(Touch_Pos.y<=F6_BUTTON_Y2)
						...
						else if((Touch_Pos.x>=PRO_NUM_TABLE_X1)&&(Touch_Pos.x<=PRO_NUM_TABLE_X2 ))//middle page table
							{
								if(SetModeEnbByCode_flag==1)//密码打开
									{
									TouchLnie_num=0;
									row_cou=0;
						...
						{
						if((Touch_Pos.y>=BACK_BUTT_Y1)&&(Touch_Pos.y<=BACK_BUTT_Y2))// stop + back + forwd
							{
							if(SetModeEnbByCode_flag==1)//密码打开
								{
				...
				case MANUAL_IO_TEST_MODE:// IO 测试模式--------
					if(Touch_Pos.y<=F6_BUTTON_Y2)
						...
						if((Touch_Pos.x>=F1_BUTTON_X1)&&(Touch_Pos.x<=F1_BUTTON_X2))  //F1,2,3,4子模式
							...
							{
							if(WorkMode==MANUAL_TOOLS_MODE)//手动 参数
								{
								TouchLnie_num=0;
								row_cou=0;
								while(row_cou<14)
									{
									if((Touch_Pos.x>(SET_DATA_ITEM_X1+(PARA_COL_WIDE*(row_cou/7))))&&(Touch_Pos.x<(SET_DATA_ITEM_X2+(PARA_COL_WIDE*(row_cou/7)))))	
										{
										if((Touch_Pos.y>(SET_DATA_ITEM_Y1+(PARA_ROW_GAP*(row_cou%7))))&&(Touch_Pos.y<(SET_DATA_ITEM_Y2+(PARA_ROW_GAP*(row_cou%7)))))
											{
											Touch_code=	LINE_SEL_KEY_CODE;	//触摸行选择
											TouchLnie_num=row_cou+1;	
											}
							...
							else if(WorkMode==MANUAL_CLOCK_MODE)//手动 时间
								{
								TouchLnie_num=0;
								row_cou=0;
								while(row_cou<5)
									{
									if((Touch_Pos.x>(SET_TIME_ITEM_X1))&&(Touch_Pos.x<(SET_TIME_ITEM_X2)))	
										{
										if((Touch_Pos.y>(SET_TIME_ITEM_Y1+(CLOCK_GAP_Y*row_cou)))&&(Touch_Pos.y<(SET_TIME_ITEM_Y2+(CLOCK_GAP_Y*row_cou))))
											{
											Touch_code=	LINE_SEL_KEY_CODE;	//触摸行选择
											TouchLnie_num=row_cou+1;	
											}
...
LongTouchDis_flag = 1;
if((Touch_code!=ADJUST_KEY_CODE)&&(Touch_code!=NEXTKNIFE_KEY_CODE) //后退按键	//前进按键
	&&(Touch_code!=FAST_BACK_KEY_CODE)&&(Touch_code!=FAST_AHEAD_KEY_CODE))   //快进     //快退
	{
	jog=0;
	}
else
