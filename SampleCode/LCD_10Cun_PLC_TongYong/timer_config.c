...
void Timer2_Config(void)
{
    ...
    outpw(REG_CLK_PCLKEN0, inpw(REG_CLK_PCLKEN0) | (1 << 6)); // Enable ETIMER0 engine clock
    // Set timer frequency to 1HZ
    ETIMER_Open(2, ETIMER_PERIODIC_MODE, 1000);

    // Enable timer interrupt
    ETIMER_EnableInt(2);
    sysInstallISR(HIGH_LEVEL_SENSITIVE | IRQ_LEVEL_1, ETMR2_IRQn, (PVOID)ETMR2_IRQHandler);
    sysSetLocalInterrupt(ENABLE_IRQ);
