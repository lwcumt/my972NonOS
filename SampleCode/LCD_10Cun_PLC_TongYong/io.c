
#include"io.h"

#include "spi_config.h"
#include "drv_glcd.h"
#include "uart_config.h"

#include "pic_exec.h"

#include "app_plc.h"

void IO_Init(void)
{
	
RLOUT1_ON;
RLOUT2_ON;
RLOUT3_ON;
RLOUT4_ON;
					#ifdef PLC_3U
							FX_Plc_EnbAutoCut_Flag=1;
						#endif	
RLOUT5_ON;
RLOUT1_1_ON;
	#ifdef PLC_3U
	FX_Plc_EnbCut_Flag=1;
#endif
OOUT1_ON;
OOUT2_ON;
OOUT3_ON;
OOUT4_ON;
OOUT5_ON;


RLOUT1_DirInit;
RLOUT2_DirInit;
RLOUT3_DirInit;
RLOUT4_DirInit;
RLOUT1_1_DirInit;
RLOUT5_DirInit;

OOUT1_DirInit;
OOUT2_DirInit;
OOUT3_DirInit;
OOUT4_DirInit;
OOUT5_DirInit;


/*
RLOUT1_OFF;
RLOUT2_OFF;
RLOUT3_OFF;
RLOUT4_OFF;
RLOUT5_OFF;
RLOUT1_1_OFF;

OOUT1_OFF;
OOUT2_OFF;
OOUT3_OFF;
OOUT4_OFF;
OOUT5_OFF;
*/


M_DOUT_OFF;
M_POUT_OFF;

LED2_DirInit;
LED2_OFF;
  
BellOUT_DirInit;
	
M_POUT_DirInit;
M_DOUT_DirInit;
	
	M_ZIN_DirInit;
  OIN1_DirInit;
  OIN2_DirInit;
  OIN3_DirInit;
  OIN4_DirInit;
  OIN5_DirInit;
  OIN6_DirInit;
  OIN7_DirInit;//输入
  OIN8_DirInit;
  OIN9_DirInit;
  OIN10_DirInit;
  OIN11_DirInit;
  OIN12_DirInit;
  OIN13_DirInit;
  OIN14_DirInit;
	
	
}

void IO_Test(void)
{

}


void CheckOp_IN(void)
{
if(M_ZIN_PIN)
	RLOUT1_OFF;
else
	RLOUT1_ON;

if(OIN1_PIN)
{
	RLOUT2_OFF;
	
}
else
	RLOUT2_ON;
	

if(OIN2_PIN)
	RLOUT3_OFF;
else
	RLOUT3_ON;

#ifdef PLC_3U
if(Press_Sensor_Status)
	RLOUT4_OFF;
else
	RLOUT4_ON;
#else

if(OIN3_PIN)
{
	RLOUT4_OFF;
	
}
else
{
	RLOUT4_ON;

}

#endif



if(OIN4_PIN)
{
	RLOUT1_1_OFF;
							#ifdef PLC_3U
							FX_Plc_EnbCut_Flag=0;
						#endif
}
else
{
	RLOUT1_1_ON;
	#ifdef PLC_3U
		FX_Plc_EnbCut_Flag=1;
	#endif
}


if(OIN5_PIN)
	RLOUT5_OFF;
else
	RLOUT5_ON;

if(OIN6_PIN)
	OOUT1_OFF;
else
	OOUT1_ON;


if(OIN7_PIN)
	OOUT2_OFF;
else
	OOUT2_ON;


if(OIN8_PIN)
	OOUT3_OFF;
else
	OOUT3_ON;

if(OIN9_PIN)
	OOUT4_OFF;
else
	OOUT4_ON;

if(OIN10_PIN)
	OOUT1_OFF;
else
	OOUT1_ON;


if(OIN11_PIN)
	OOUT2_OFF;
else
	OOUT2_ON;


if(OIN12_PIN)
	OOUT3_OFF;
else
	OOUT3_ON;

if(OIN13_PIN)
	OOUT4_OFF;
else
	OOUT4_ON;

}



void I2C_Init(void)
{
int rtval;
    /* Configure multi function pins to I2C0 &   I2C1 */
    outpw(REG_SYS_GPG_MFPL, (inpw(REG_SYS_GPG_MFPL) & ~0xff00) | 0x8800);



    i2cInit(1);

    rtval = i2cOpen((PVOID)I2CNUM_1);
    if(rtval < 0) {

		rtval=100;
      //  sysprintf("Open I2C0 error!\n");
      //  goto exit_test;
    }
    i2cIoctl(I2CNUM_1, I2C_IOC_SET_DEV_ADDRESS, PCF8563_DEV_ADD, 0);  /* On 910 EV board, set 0x50 for I2C0 */
    i2cIoctl(I2CNUM_1, I2C_IOC_SET_SPEED, 400, 0);
}






void I2C_Test(void)
{
	
Sec=0;
Min=0x35;
Hour=0x0;
Dom=0x4;
Month=0x7;
Year=0x17;
write_time();
}



void Sys_DevInit(void)
{
IO_Init();
Spi_Init();
I2C_Init();
Uart3_Init();//PLC通讯  UART 3
	
Uart9_Init();//键盘UART9  
	
GLCD_Init();

}


//--------20191108
void AirSwitch_Work(void)
{
switch(MotorType)
	{
	case MOTOR_FREQ_5_SPD_RST: //----5档速
		
		break;
	case MOTOR_FREQ_3_SPD: //----3 档速	
	case MOTOR_FREQ_3_SPD_RST: //----3 档速+复位	
		RLOUT3_OFF;
		break;
	case MOTOR_AC_SERVO:
	case MOTOR_AC_SERVO_REV:	
		RLOUT2_OFF;
		#ifdef PLC_3U
			FX_Plc_EnbQiFa_Flag=0;
		#endif	
		break;
	}	
}
void AirSwitch_Stop(void)
{
switch(MotorType)
	{
	case MOTOR_FREQ_5_SPD_RST: //----5档速
		
		break;
	case MOTOR_FREQ_3_SPD: //----3 档速	
	case MOTOR_FREQ_3_SPD_RST: //----3 档速+复位	
		RLOUT3_ON;
		break;
	case MOTOR_AC_SERVO:
	case MOTOR_AC_SERVO_REV:	
		RLOUT2_ON;
		#ifdef PLC_3U
			FX_Plc_EnbQiFa_Flag=1;
		#endif		
		break;
	}	
}

void AutoPress_Work(void)
{
switch(MotorType)
	{
	case MOTOR_FREQ_5_SPD_RST: //----5档速
		
		break;
	case MOTOR_FREQ_3_SPD: //----3 档速	
	case MOTOR_FREQ_3_SPD_RST: //----3 档速+复位	
		
		break;
	case MOTOR_AC_SERVO:
	case MOTOR_AC_SERVO_REV:	
		RLOUT3_OFF;
		break;
	}
}
void AutoPress_Stop(void)
{
switch(MotorType)
	{
	case MOTOR_FREQ_5_SPD_RST: //----5档速
		
		break;
	case MOTOR_FREQ_3_SPD: //----3 档速	
	case MOTOR_FREQ_3_SPD_RST: //----3 档速+复位	
		
		break;
	case MOTOR_AC_SERVO:
	case MOTOR_AC_SERVO_REV:	
		RLOUT3_ON;
		break;
	}
}

void EnblePress_Work(void)
{
switch(MotorType)
	{
	case MOTOR_FREQ_5_SPD_RST: //----5档速
		
		break;
	case MOTOR_FREQ_3_SPD: //----3 档速	
	case MOTOR_FREQ_3_SPD_RST: //----3 档速+复位	
		
		break;
	case MOTOR_AC_SERVO:
	case MOTOR_AC_SERVO_REV:	
		RLOUT5_OFF;
		break;
	}	
}
void EnblePress_Stop(void)
{
switch(MotorType)
	{
	case MOTOR_FREQ_5_SPD_RST: //----5档速
		
		break;
	case MOTOR_FREQ_3_SPD: //----3 档速	
	case MOTOR_FREQ_3_SPD_RST: //----3 档速+复位	
		
		break;
	case MOTOR_AC_SERVO:
	case MOTOR_AC_SERVO_REV:	
		RLOUT5_ON;
		break;
	}	
}