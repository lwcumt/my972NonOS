...
if((Knife_order<=MAX_Knife)&&(Knife_order>1))	//刀数据 未满
	...
	if(InputSize>0)
		...
		GetOneKnife_datas(Pro_data.pro_num, Knife_order-1, pKnife_data);// 读取 前刀 尺寸
...
Int16U x,y,gap,back_color,line_color,tx_wide=122;
...
BellOn_Enb();
		...
		{
		if((Manual_Knife_Status==1)||(Fast_Knife_Status==1)||(Motor_Knife_Status==1)||(Knife_Change_Status==1))
			{
			return;			
			}			
		}
...
switch(keycode)
					...
					case AUTO_MODE://自动--  主画面下
						if(MotorType>=MOTOR_AC_SERVO)
							...
							if(ElectricWheelOn_flag==1)	//手轮  有效时
								{
									/***xxxxxxxxxxxxxxxxxxxxx 20191030
				 ...
				 if(Run_flag==0)
					...
					switch(WorkMode)
						...
						case MANUAL_TOOLS_MODE:// 手动 参数模式
							SelectSetDatas_Exec(TouchLnie_num-1 );//参数项触摸选择
							TouchLnie_num=0;
							break;
						case MANUAL_CLOCK_MODE://手动 时钟模式
							SelectSetTime_Exec(TouchLnie_num-1  );//clock项触摸选择
							TouchLnie_num=0;
							break;
						}
					}
				...
				switch(WorkMode)
					...
					case READ_PRO_MODE:// 程序查找 主画面下
						if(WorkMode==AUTO_MODE)
							{
							if(Knife_order>OnePro_head.pro_knifeSUM)
								AutoModeKnifeOrder_temp=Knife_order=OnePro_head.pro_knifeSUM;
							else
								AutoModeKnifeOrder_temp=Knife_order;
							}
					...
					switch(WorkMode)
						...
						case READ_PRO_MODE:// 程序查找 主画面下
							if(WorkMode==MANUAL_MODE)
								{
								Knife_order	=AutoModeKnifeOrder_temp;
								}
							else
								Knife_order=1;//
							Flg_chg=1;
			...
			if(Run_flag==0)
				...
				switch(WorkMode)
					...
					case MANUAL_TOOLS_MODE:// 手动 参数模式
				  	...
				  	FX_PLC_ChangeAndSave_ParaData();//保存FX_PLC 参数//F1退出时的保存
					...
					case MANUAL_RULER_MODE://手动 基准模式
						if(num_cou>0)
							{
							if(SetModeEnbByCode_flag==1)//pass code check
								{
								if(CheckInputNums()==1)
									{
				...
				// // if(ButtonTouch_mode==0)
					...
					switch(WorkMode)
						...
						case MANUAL_IO_TEST_MODE: //IO mode
							...
							PromptDis_flag=1;
							ACServoAlarmExec_flag=0;
							SetModeEnbByCode_flag=0;//pass code check
							break;
						}
					// // }
				// // else if(ButtonTouch_mode==TOU_MODE_CALC)  //计算器 弹出模式
							...
							case 3://语言
								...
								switch(Language)
									...
									case CHN_HAN:
										...
										GLCD_SetWindow_Fill( x, y, x+90, y+24, back_color,back_color);//clear text win//120
    ...
    case UP_KEY_CODE:    ///////////运行按键*******************  
      qianjinflag=0;
        ...
        if((WorkStart_flag==1)&&  (AllPressUp_flag==1)&&(AllKnifeUp_flag==1))
			{
				...
				switch(WorkMode)
					...
					//case TEACH_MODE://示教画面下
						...
						AutoRun_start();   //运行至 光标处
							break;
					case TEACH_MODE://示教 模式
					// case AUTO_MODE://自动  主画面下20180403
			 ...
			 switch(WorkMode)
					...
					case AUTO_LABEL_MODE:		//标签 模式**********************************
						if(num_cou>0)
						   ...
						   if((Total_size*Label_size*Waste_size)!=0)//设定有效
								{
									Cal_Form_Manual_Exit();//退出弹出大计算器窗口
					...
					case READ_PRO_MODE:	//程序选择*****************************	  
						if(SetModeEnbByCode_flag==0)//
							{
						//	if(num_cou>0)
							//	{
							//	if(CheckInputNums()==1)	
						//{
						///	if(InputSize==SET_PASS_CODE) //密码正确	
								//{
					...
					case MANUAL_RULER_MODE://手动 基准模式
						if(num_cou>0)
							...
							if(CheckInputNums()==1)
								{
								if(SetModeEnbByCode_flag==0)//pass code check
									{
									if(InputSize==SET_PASS_CODE) //密码正确	
										{
										SetModeEnbByCode_flag =1;
...
//////												}
										...
										case 8:	//关气阀 距离
											input_temp/=100;
											if((input_temp<=10000)&&(input_temp>=200))
												{
												if(input_temp<200)
													AirOffDistance=200;
												else
										...
										case 10:
											if(MotorType>=MOTOR_AC_SERVO)  //*************交流 伺服
												{
												if((input_temp>=1)&&(input_temp<=30))
													{
													KnifeDownTime=input_temp;
												//	SaveACMotorSpeed();   //交流伺服 速度
												//	GetAC_MotorSpeed();  //交流伺服  速度判定
														
													 PLC_Save_Config();//保存下刀时间j
										...
										case 11:
											if(MotorType>=MOTOR_AC_SERVO)  //*************交流 伺服
												{
												if((input_temp>=1)&&(input_temp<=180))
													{

														if(MotorType>=MOTOR_AC_SERVO)  //*************
															{
															FX_PLC_Set_Process(Para_sel,InputSize);
															}
									...
									switch(Para2_sel)
										...
										case 3://press time
											if(MotorType>=MOTOR_AC_SERVO)  //*************
												{
												if((InputSize>=1)&&(InputSize<=10))	//	
													{
													AutoPressWork_TimeSet=InputSize;
									...
									if(CheckInputNums()==1)
										...
										if((InputSize>0)&&(InputSize<=300)) //数据有效 保存
											...
											if(Para_sel==0)  //第一次 时间
												{
												Setting_User_Count[0]=InputSize;
												Setting_User_Count[1]=0;//重新赋值
												SaveWorkDays();//起始 时间保存	
...
void FindEmptyPro(void)// 查找空程序20180523
{
Int16U p,	i,t,flag;
volatile Int32U date[MAX_PROG+2],temp;
pInt8U pdata;
...
for(p=1;p<=MAX_PROG;p++)
	{
	GetPro_headInfo(p,  pdata);//sdram 读一个程序头(程序名+ 时间 +刀数 )
	if(pro_Head.pro_knifeSUM<=1)
		{
		GetOneKnife_datas(p, 1 , (pInt8U)&Knife_datas);//取出一刀 数据
		if(Knife_datas.size==0)
			break;
		}
	}
	if(p>MAX_PROG)
		p=1;
//***************display 
	if(ProNumGroup[prog_order]!=p)	//判断 程序 号码 非当前
		{
		for(i=1;i<=MAX_PROG;i++)
			{
			if(ProNumGroup[i]==p)
...
void ReadAheader(void)//读取提前量 改成裁切总数了  2025 03
{
pInt8U psize;
Int32U add;
Int16U n;
...
psize=(pInt8U)&Setting_User_Count;

FastRead_Datas_Start(add);
for(n=0;n<4;n++)	
//for(n=0;n<6;n++)//原来保存天数
	{
	*psize++=ReadFlash_Datas();
	}
FlashChip_Dis;
...
psize=(pInt8U)&Setting_User_Count;
	//psize=(pInt8U)&Start_days;
Write_25pe_data(psize, add, 4);
//Write_25pe_data(psize, add, 6);	//原来保存天数
}
...
FlashChip_Dis;
if((AirOffDistance>10000)||(AirOffDistance<200))
	AirOffDistance=200;
}
...
SuperPassCode=PASSCODE_forSupermula(PassCheck);	
}
...
void MachineUserCount_Check(void)//机器使用次数密码检查
{
ReadWorkDays();//先读取一下数据
	
if((Setting_User_Count[0]==0xA55A)&&(Setting_User_Count[1]==0x5AA5))	
{//已超级密码解锁，退出	
	PassCodeEn_flag=0;//无须再输入密码
	return;
}

if(Setting_User_Count[0]>200)
		{//超出初时值。赋初值，无效
			Setting_User_Count[0]=0;
			Setting_User_Count[1]=0;
			SaveWorkDays();
		}
		
if(Setting_User_Count[0]>0)
		{
			Setting_User_Count[1]++;
			if(Setting_User_Count[1]>=Setting_User_Count[0])
				{
				PassCodeEn_flag=1;
				}
...
switch(Language)
	...
	case PORTUGAL:	//葡萄牙
		...
		Printf24("Cancelar",DEL_ALL_CANCEL_X,DEL_ALL_CANCEL_Y,BLACK_COLOR,1,fill_color);
			break;
	case THAILAND:
		//unsigned short const Thai_Font17_Pic_18[];   ////18：是否删除全部数据？???????????????????????????
		Thai_Draw(Thai_Font17_Pic_18,DEL_ALL_HINT_X,DEL_ALL_HINT_Y,BLACK_COLOR,0,fill_color);
	
	//unsigned short const Thai_Font17_Pic_14[];   ////14：确认 ????
		Thai_Draw(Thai_Font17_Pic_14,DEL_ALL_OK_X,DEL_ALL_OK_Y,BLACK_COLOR,1,fill_color);
...
switch(PageKf_Size_flag)
	...
	case PAGE_5_LINE: //5 line 
			PageKf_Size=5;
			Kf_Line_HIGH=50;//60//行高
			txtbank_y=Kf_Line_HIGH-32;
			break;
	case PAGE_7_LINE: //5 line 
			PageKf_Size=7;
			Kf_Line_HIGH=32;//40;//行高
			txtbank_y=Kf_Line_HIGH-32;
			break;
	}
}
...
void SelectSetDatas_Exec(Int8U cursor)//参数项触摸选择
{
Int8U p1_nums=9,p2_nums=2;
if(ButtonTouch_mode==0)  //	 非 触摸计算器 下
	{
	switch(MotorType)
		{
...
void SelectSetTime_Exec(Int8U cursor)//clock项触摸选择
{
if(cursor<=4)
	{
	if(cursor!=Para_sel)
