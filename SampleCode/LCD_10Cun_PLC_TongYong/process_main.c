...
void LoadWorkMode_Process(void);
...
void Disp_KnifeDown_Menu(TOUCH touch);
...
MotorType=3;//MOTOR_AC_SERVO


Timer2Init();//Ms_Flag 对应定时器  新开启的 2024-01
...
GLCD_SetWindow(0,0,C_GLCD_H_SIZE-1,C_GLCD_V_SIZE-1,RED_COLOR);//
...
////  //uart0
////UART0_Init();//PC通讯 UART3 
////UART3_Init();//键盘通讯  UART9 
...
MotorType=MOTOR_AC_SERVO;//强制交流伺服
...
ReadAutoPressWork_TimeSet();//自压时间

PassCodeEn_flag=0;//清0 防止原来的程序影响到	
Password_Form_Init();
...
//Language=THAILAND;
//	GLCD_Buffer_Fill(BLUE_COLOR);
//LoadWorkMode_Process();
	
while(1)
   {
   //ACMotor_GetZpulse();
 //  GLCD_SetWindow(0,0,799,599,RED_COLOR);
   if(Ms_Flag!=0)
	  {
			//		BellOn_Enb();
//	CursorSize_dis(1000);//cishu);
			
			#ifdef PLC_3U
				Check_MainMotor_Run_Status();//检测主电机状态和手动换刀处理
...
//	sprintf(string_buf,"%d",(int)CurrentSize);
//	Printf24((unsigned char*)string_buf,100,300,RED_COLOR,1,cpGui_ColorChange(PLC_HINT_DIS_BAK_COLOR));	
		
		Check_FX_PLC_Motor_Run_Status();	//主机启动判断  检测 拉刀左、右，换刀左、右 
		
	//	cishu=5000;
		Disp_KnifeDown_Menu(Touch_Pos);
		
//CursorSize_dis(cishu);
		#ifdef CAP_TOUCH
		...
		switch(WorkMode)
			...
			case TEACH_MINUS_MODE://示教 F4--
				...
				switch(MotorType)
					...
					if(Ori_Sensor_Status)
					{
						yuandianSensorInMachine_dis(0);
					}
					else
					{
							yuandianSensorInMachine_dis(1);
					}					
					#else
					if(OIN1_PIN)
					{
			...
			//						Printf16("刀低无法运行!",250,180,RED_COLOR,0,0xffff);
								 }
							 else
								 {
								...
								{
									Printf16("             ",250,165,HUISE_COLOR,1,0xef5f);
								}
								
								 if(	WorkMode==MANUAL_MODE)
									{
									Printf16("             ",250,165,WHITE_COLOR,1,0xffff);
									}
...
//									Printf16("压低无法运行!",430,210,RED_COLOR,0,0xffff);
								...
								PressSensorInMachine_dis(1);//压纸 传感器位置 示意
									if(	WorkMode==AUTO_MODE)
									{
									Printf16("             ",430,210,HUISE_COLOR,1,0xef5f);
									}
		             if(	WorkMode==MANUAL_MODE)
									{
									Printf16("             ",430,210,WHITE_COLOR,1,0xffff);
									}
...
//-----------	clr auto-press
if(AutoPressWork_flag==1)
	{
	AutoPressWork_Time++;	
	if(AutoPressWork_Time>5*AutoPressWork_TimeSet)//3s
		{
		AutoPressWork_Time	=0;
		AutoPressWork_flag=0;
		AutoPress_Stop();
		}
	}
}
...
{
volatile Int8U flags,air_flag,enb_flag=1;
if((WorkMode==SELF_CUT_MODE)||(WorkMode==AUTO_MODE)||(WorkMode==TEACH_MODE))  //自刀,自动 示教主模式
	{
	flags=OneKnife_data.k_flag&0x0f;
	air_flag=OneKnife_data.k_flag&0xF0;
	switch(flags)
		{
		case PUSH_FLAG://推纸 标志  则自动走到下一刀
			PressFlagRunCou=0;	
			// AutoRun_AutoPush_exec();			------20200420
			AutoRoute_flag=1;
			CursorDown_inTable();//**** 光标下移 显示 数据
			RunTo_cursor();//    运行到下一刀
			AutoPushRun_flag=0;
		...
		case KNIFE_FLAG: //刀 标志
			...
			break;
		case SORT_FLAG:   // 2   理纸
			// if(MotorType>=MOTOR_AC_SERVO)++++++20191230
				{
				PressFlagRunCou=0;		
				enb_flag=0; 
				RLOUT1_ON; RLOUT1_1_ON; // 
				#ifdef PLC_3U
					FX_Plc_EnbCut_Flag=1;
				#endif
				EnblePress_Stop();//允许压   20191119断开 	
				}
			// else
				// {
				// enb_flag=1;	
				// }
			break;	
		case PRESS_FLAG:   // 3   压纸
			if(MotorType>=MOTOR_AC_SERVO)
				{
				enb_flag=0;	
				RLOUT1_ON; RLOUT1_1_ON; // 断开 
						#ifdef PLC_3U
	FX_Plc_EnbCut_Flag=1;
#endif
				EnblePress_Work();//闭合
				PressFlagRunCou++;
				if(PressFlagRunCou>1)
					{
					AutoPress_Work();//闭合 ziya
					AutoPressWork_Time=0;
					AutoPressWork_flag=1;
					}
				}
			else
				{
			...
			break;
		// case NOAIR_FLAG|FLAG_FLAG:
		default:	
			PressFlagRunCou=0;		
			break;
		}
	if(enb_flag==1)//无标记 闭合   允许压+允许裁切
		{
		if(ACServoDrvErr_flag==0)
			{
			RLOUT1_OFF; RLOUT1_1_OFF;   // 允许裁切
		...
		//***********
	if(air_flag==AIR_FLAG)//气垫 标志	气阀闭合
		{
		AirSwitch_Work(); //闭合
		AirOffOK_flag=0;
		}
...
OneKnife_data.k_flag=NOAIR_FLAG|FLAG_FLAG;  // 标志 init
...
switch(StartCheck_mode)
	...
	case 16:  //Z脉冲 为高
			...
			if((OutFront_limit==0)&&(OutMiddle_limit==0)&&(Wheel_limit==0))
				...
				//xxxxxxxxxxxxxx20200328
			if((OutFront_limit==1)||(OutMiddle_limit==1))	//极限异常
				{
				MoveStop();
				StartCheck_mode=0;
				}	
	...
	case 17:	         //停止后   进入工作画面
			...
			if(stop_time>=50)
				...
				// CurrentSize=OriginalSize;//Current_last=CurrentSize;
	...
	switch(MotorType)
		...
		case MOTOR_AC_SERVO_REV:
				...
				break;
					...
					//---------------气阀控制
		if((WorkMode== AUTO_MODE)||(WorkMode==TEACH_MODE)||(WorkMode==SELF_CUT_MODE))//20191128
			{	
				//取消了关气距离，改成保养次数
...
////////					{
////////					if((qianjin==1)||(Forward_flag==1))//向前 
////////						{
////////						if(size_temp<=AirOffDistance)
////////							{
////////					    	AirSwitch_Stop();	//气阀断开 20191108
////////								volatile pInt8U pdata=(pInt8U)&OneKnife_data;
////////							GetOneKnife_datas(Pro_data.pro_num, Knife_order, pdata);//
////////							if(OneKnife_data.k_flag&0xf0==AIR_FLAG)
////////								{
...
{
	if(Language==CHN_HAN)
			Printf24("PLC 未连接！请检查...",START_PROMPT_X2,START_PROMPT_Y2+5,RED_COLOR,1,0xffff);
	else
		{
		Printf24(" PLC is not LINK ,Please Check...",START_PROMPT_X2,START_PROMPT_Y2+5,RED_COLOR,1,0xffff);		
...
switch(StartCheck_mode)
	...
	case 6:  //前进   中-----后极限异常 进入			
			if(OutFront_limit==1)//前极限
				{
				ACServoStop();
							//							Printf24("StartCheck_mode=6;OutFront_limit=1原点碰到停下来了",PASSCODE_X,PASSCODE_Y,RED_COLOR,1,0xffff);		
				StartCheck_mode=0;
				}
			else
				{
	...
	case 7:			
			if(OutFront_limit==1)//前极限
				...
				StartCheck_mode=0;
						...
						//					Printf24("StartCheck_mode=7;OutFront_limit=1原点碰到停下来了",PASSCODE_X,PASSCODE_Y,RED_COLOR,1,0xffff);
				}
			else
				{
				stop_time++;
				if(stop_time>=60000)
					...
					stop_time=0;
					ACServoStop();
				//		Printf24("StartCheck_mode=7;OutFront_limit=0 原点碰到停下来了",PASSCODE_X,PASSCODE_Y,RED_COLOR,1,0xffff);
	...
	case 9://5-----运行键按下   后          -----后极限ok 进入
			if(OutBack_limit==0)  //if((OutFront_limit==0)&&(OutMiddle_limit==0))
				...
				if(stop_time<2000)
					...
					ACServoStop(); 
					StartCheck_mode=0;	
				//		Printf24("StartCheck_mode=9;原点碰到停下来了",PASSCODE_X,PASSCODE_Y,RED_COLOR,1,0xffff);
					}
				}
			break;
	case 10://6://-----	 低速后退
			...
			//		Run_flag=1;
				...
				// if(OutBack_limit==1)// 仍然异常
					...
					StartCheck_mode=0;   //停止
						...
						//				Printf24("StartCheck_mode=11;超出时间差 原点碰到停下来了",PASSCODE_X,PASSCODE_Y,RED_COLOR,1,0xffff);		
					// }
				}
	...
	case 12://7:  //前进------检测 z  信号   停止
			...
			if((OutFront_limit==0)&&(OutMiddle_limit==0))
				...
				ACServoStop(); 
				StartCheck_mode=0;
					//					Printf24("StartCheck_mode=12;超出时间差 原点碰到停下来了",PASSCODE_X,PASSCODE_Y,RED_COLOR,1,0xffff);							
				}	
			break;
	case 13://8:
			stop_time++;
...
if(MotorType == MOTOR_AC_SERVO)	//	20190523
	...
	if(qianjincn>houtuicn)
		{  
		CurrentSize=OriginalSize-((qianjincn-houtuicn));
		} 
	else
		{  
		CurrentSize=OriginalSize+((houtuicn-qianjincn));
		} 
...
if(houtui==1&&flagtingji8==1&&flagyd==0)
    {   
	 ...
	 while(i--);
	   ...
	   rollsteps=ACSERVER_BACK_OFFSET;
	   step_ground=11; 
    ...
    {  
		...
		if(flagbeep==1&&qianjin==1)
			...
			*/
			...
			#ifdef PLC_3U
					if(Knife_Sensor_Status)   //如果第一路光藕输入为高?
        {
        if(!Press_Sensor_Status)   //如果第一路光藕输入为高?
            {     
             flagshoulun=0;
            }
...
if(Board_TestMod==0)//非功能测试模式
	...
	if(ACServoDrvErr_flag==0)
		{
		if(OIN9_PIN!=0)
			ACServoDrvErr_flag=1;  //交流伺服  警报检测
		}	
	else
		{        //警报  处理
		if(ACServoAlarmExec_flag==0)
			...
			if(WorkStart_flag==0)  
				...
				switch(Language)
					{
					case CHN_HAN:
							Printf24("交流伺服驱动器报警!",START_PROMPT_X2,START_PROMPT_Y2+5,RED_COLOR,1,0xffff);
								break;
					case ENGLISH:
					case INDONSIA:	//印尼
				...
				switch(Language)
					{
					case CHN_HAN:
						Printf24("交流伺服驱动器报警!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
								break;
					case ENGLISH:	
					case INDONSIA:	//印尼	
				...
				if((Run_flag==1)||(GoFrontEn_flag==1)||(GoBackEn_flag==1))
					ACServoStop();  //交流伺服	停止
				//RLOUT1_ON;    // 允许裁切断开
				// ASCII_Printf(Err_XXX,192+1,32,48,ASCII_32x48, CURR_SIZE_X,CURR_SIZE_Y,RED_COLOR,CURREN_STEP_BACK_COLOR);
...
if((OneKnife_data.size>=Size_MIN_num)&&(OneKnife_data.size<=Size_MAX_num))//数据有效
	...
	if(size_now==OneKnife_data.size)
		...
		switch(flags)
			{
			// case SORT_FLAG: //理纸 标志  则自动走到下一刀  20191119 去掉
	...
	case SELF_CUT_MODE:
		if((GoFrontEn_flag==0)&&(GoBackEn_flag==0))
			...
			if(ReadAir_flag==0)	
				...
				ReadAir_flag=1;	
				GetOneKnife_datas(Pro_data.pro_num, Knife_order, pdata);//
				Air_flag_temp=OneKnife_data.k_flag&0xf0;	
				}
			if(Air_flag_temp==AIR_FLAG)	
				{
				if(PressUp_flag==1)//up
					{
					// if(AirOffOK_flag==0)
						AirSwitch_Work();// 气阀闭合
					}
...
unsigned char KinfeDown_Cnt_Pop_Status=0;

LABEL KnifeDownCnt_FormLabel={Label_Type,
							TRUE,
							50+5,(32-24)/2+50,
						128,24,	
						0xffffff,
...
LABEL KnifeCHANGE_Label={Label_Type,
							TRUE,
							50+5,(32-24)/2+50+100,
						128,24,	
						RED_COLOR,
			...
			BUTTON KnifeDownBTOk={Button_Type,\
								TRUE,
								350,360,\
								100,48,\
								(240<<16)+(240<<8)+240,\
...
{
	int len;
if((KinfeDown_Cnt_Pop_Status==0)&&(Run_flag==0))
	{
	if(cishu>=AirOffDistance)
			{
				
				KinfeDown_Cnt_Pop_Status++;
				...
				len=strlen(KnifeDownCnt_FormLabel.Text);
				len*=12;

				KnifeDownCnt_FormLabel.Location.x=(800-len)/2;
				KnifeDownCnt_FormLabel.Location.y=234+4-84;

				cpGui_CreateLabel(KnifeDownCnt_FormLabel);
				
				len=strlen(KnifeCHANGE_Label.Text);
				len*=12;

				KnifeCHANGE_Label.Location.x=(800-len)/2;
				KnifeCHANGE_Label.Location.y=234+4-84+100;
				cpGui_CreateLabel(KnifeCHANGE_Label);
				
				cpGui_CreateButton(&KnifeDownBTOk);

			}
	}
else
	{
		if(KinfeDown_Cnt_Pop_Status==1)
		{//等待关闭
			...
			//if(cishu>0)
					...
					if((touch.x>=KnifeDownBTOk.Location.x)&&(touch.x<KnifeDownBTOk.Location.x+KnifeDownBTOk.Size.width)&&\
							(touch.y>=KnifeDownBTOk.Location.y)&&(touch.y<KnifeDownBTOk.Location.y+KnifeDownBTOk.Size.high))
						{
