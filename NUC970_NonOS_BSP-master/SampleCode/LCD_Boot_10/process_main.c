...
CurrentArea_save4(START_JOG_BACK_X, START_JOG_BACK_Y,   START_JOG_WIDE, START_JOG_HIGH);//jog 提示
...
//		TouchScan_Uart();
	  ...
	  if((MotorType<MOTOR_DC_SERVO)||(MotorType>=MOTOR_AC_SERVO)) //变频器 3种+AC  servo
		 {
							...
							if(CutPaper_IN1==0) //  刀 下--------------------
								 ...
								 if(KnifeDown_flag==0)
									...
									if(InputTouch_flag==0)
										Knife_Down_dis( KNIFE_X,  KNIFE_Y);//刀下
									}
								if(Run_flag==0)
									{
							 ...
							 if(PressPaper_IN!=0) //压纸 下--------------------
								 ...
								 if(PressDown_flag==0)
									...
									PressUp_flag=0;
									if(InputTouch_flag==0)
										Press_Down_dis(PRESS_X,PRESS_Y);//压纸 下
									// switch(MotorType)
										// {
					...
					case MOTOR_DC670_SERVO_ZHU:	//*******************
							if(CutPaper_IN1!=0) //  刀 下--------------------
								 ...
								 if(KnifeDown_flag==0)
									...
									KnifeUp_flag=0;
									if(InputTouch_flag==0)
										Knife_Down_dis( KNIFE1_X,  KNIFE1_Y);//刀下
									}
								if(Run_flag==0)
									{
									if(PaperCutEnb_signal==2)
								 ...
								 if(KnifeUp_flag==0)
									...
									KnifeUp_flag=1;
									if(InputTouch_flag==0)
										Knife_Up_dis(KNIFE1_X,KNIFE1_Y);//刀上
									if(PressStopDis_flag==1)//压纸停机 提示
										{
										PressStopDis_flag=0;
										PromptNum_dis();//输入范围
										...
										if((GoFrontEn_flag==0)&&(GoBackEn_flag==0))
											...
											if(MotorType<MOTOR_DC_SERVO)
												...
												if(ToTargetStop_flag==1)
													{
													if(InTargetPosit_flag==1)	//已停在 目标位置
					...
					if(Run_flag==0)
						...
						if(WorkMode == AUTO_MODE)
							...
							if(PressCircle_mode== 2)
								...
								if(OneKnife_data.k_flag==PRE_AUTO_FLAG) //压纸 标志  则自动走到下一刀
									{
			...
			if(RunStop_flag==1)
				...
				switch(MotorType)
					...
					case MOTOR_FREQ_3_SPD_RST:
						...
						if(temp<MARGIN_SIZE)  //误差允许范围  ------自锁确认
							{
							//Run_flag=0;
...
if((WorkMode==SELF_CUT_MODE)||(WorkMode==AUTO_MODE)||(WorkMode==TEACH_MODE))  //自刀,自动 示教主模式
	{
	if(OneKnife_data.k_flag==PUSH_FLAG) //推纸 标志  则自动走到下一刀
		{
		AutoRun_AutoPush_exec();
		OneCutRoutine_flag=1;
		}
		...
		if(OneCutRoutine_flag==1) //一个裁切循环 后
			...
			else if(WorkMode==AUTO_MODE)  //自动模式下   裁切立刻停止
				{
				if(AutoCutStart_flag==1)
					{
					BellOUT_OFF;
	...
	if(OutBack_limit==0)
			...
			if(outLimB_time>=20)
				...
				if(WorkStart_flag==1)
					...
					if(senor==1)
						...
						//--------------开机jog 按钮显示------
					FrontPage_Up_dis( START_JOG_FORW_X,  START_JOG_FORW_Y);//前进up
					BackPage_Up_dis( START_JOG_BACK_X,  START_JOG_BACK_Y);//后退up	
		...
		if(BackLimit_IN==0)
			...
			if(WorkStart_flag==1)
				...
				{
				//---------------------------清除  jog 提示
				CurrentArea_recover4(START_JOG_BACK_X, START_JOG_BACK_Y,   START_JOG_WIDE, START_JOG_HIGH);//jog 提示
				...
				if(GoFrontEn_flag==1)
					...
					if(Wheel_limit!=1)
						...
						if(PressStop_check()==0)
							{
							sp_temp=CurrentSize-Size_MIN;
							if((sp_temp<MARGIN_SIZE)||(OutFront_limit==1))//到前限位    停止
								{
								MoveStop();
								JogRunMode=3;
								//Run_flag=0;
			...
			case 5: //低速 后退
				if(GoBackEn_flag==1)
					...
					if(Wheel_limit!=1)
						{
						sp_temp=Size_MAX-CurrentSize;
						if((sp_temp<MARGIN_SIZE)||(OutBack_limit==1))  //停止
							{
							MoveStop();
							JogRunMode=6;
							Run_flag=0;
...
if(Wheel_limit!=1)
	...
	if(Backward_flag==1)  //后退
		...
		// if((tt<MARGIN_SIZE)||(OutBack_limit==1))  //停止
		...
		if(PressStop_check()==0)
			...
			// if((tt<MARGIN_SIZE)||(OutFront_limit==1))//到前限位    停止
			if(OutFront_limit==1)//到前限位    停止
				{
				MoveStop();
				ForceStop_flag=0;
				...
				// // if(senor==0)    //原点故障
					...
					// // if(OriginalError_flag==0)
						...
						// // switch(Language)
							// // {
							// // case CHN_HAN:
									// // Printf24("原点故障!可按进-退键!",START_PROMPT_X1,START_PROMPT_Y1+5,RED_COLOR,1,0xffff);
									// // break;
							// // case ENGLISH:
					...
					if(OutBack_limit==0)
						...
						if(PushRstKey_flag==0)
							...
							if(RunDisplay_flag==0)
								{
								RunDisplay_flag=1;
								GLCD_SetWindow_Fill( REMIND_WINDOW_X, REMIND_WINDOW_Y, REMIND_WINDOW_X-1+600, REMIND_WINDOW_Y-1+30, 0xfffff, 0xffff);//提示框
								switch(Language)
									{
									case CHN_HAN:
											Printf24("电脑检测完成,请按运行键!",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,1,0xffff);
												break;
									case ENGLISH:
											Printf24("Computer testing OK, press the RUN key!",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,1,0xffff);
												break;
									case PORTUGAL:	//葡萄牙
											Printf16("Computador auto-teste estiver concluí",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,1,0xffff);
					...
					if(CodeSensorTime_AB<5000)//5s
						...
						if(ABsensor_temp<3)  //编码器故障
							...
							switch(Language)
								{
								case CHN_HAN:
										Printf24("编码器故障!",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,1,0xffff);
											break;
								case ENGLISH:
...
switch(StartCheck_mode)
	...
	case 1:
			if((OutFront_limit==0)&&(OutMiddle_limit==0))
				{
				// if((senor==1)&&(OutBack_limit==0))   //信号  正常   提示运行
		...
		if(ACServoAlarmExec_flag==0)
			...
			if(WorkStart_flag==0)  
				...
				switch(Language)
					{
					case CHN_HAN:
							Printf24("交流伺服驱动器报警!",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,1,0xffff);
								break;
					case ENGLISH:
