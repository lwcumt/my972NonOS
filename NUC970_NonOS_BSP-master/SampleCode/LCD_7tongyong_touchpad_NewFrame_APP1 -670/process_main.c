...
ReadFrontValidStatus();//前极限valid
ReadMiddleValidStatus();//2极限valid
ReadBackValidStatus();//3极限valid
					...
					// case MOTOR_AC_SERVO:
							...
							if(CutPaper_IN1==0) //  刀 下--------------------
							 ...
							 if(PressPaper_IN!=0) //压纸 下--------------------
								...
								break;
					case MOTOR_AC_SERVO:		//+++++++++++++++++++20191224++++++++++++++++++AC++++670 
							if(CutPaper_IN2==0) //  刀 下--------------------(z)
								 {
								 AllKnifeUp_flag=0;//----00000
								 CutRelease_time=0;
			...
			/****
					...
					if(PaperCutEnb_signal==1)
						...
						//	if(PressPaper_IN==0)
							...
							{
							if((MotorType!=MOTOR_DC670_SERVO)&&(MotorType!=MOTOR_DC670_SERVO_ZHU)&&(MotorType!=MOTOR_AC_SERVO))//+++MOTOR_AC_SERVO
								NewKnife_sign_mod=0;
							if(((OneKnife_sign_mod&0x3)==3)||((OnePress_sign_mod&0x3)==3)||((NewKnife_sign_mod&0x3)==3))
								{
								CutRelease_time++;
								if(CutRelease_time>4000)   //超时 裁切信号消失
									{
									CutRelease_time=0;
									OneKnife_sign_mod=0;
									NewKnife_sign_mod=0;
					...
					if(Run_flag==0)
						...
						if(WorkMode == AUTO_MODE)
							...
							if(PressCircle_mode== 2)
								...
								if(OneKnife_data.k_flag==PRE_AUTO_FLAG) //推纸 标志  则自动走到下一刀
									{
...
if(CurrentSize_temp!=CurrentSize)
	...
	if(CurrentSize_temp>CurrentSize)
		{
		MoveDirArrow_dis(0);//front
		}
	else 
		{
		MoveDirArrow_dis(1);//back
		}
	if(CurrentSize>TargetSize)
		{
