...
if(Knife_order<MAIX_Knife)	//刀数据 未满
	...
	if(InputSize>0)
		...
		if((newsize_temp<Size_MIN_num)||(newsize_temp>Size_MAX_num))//超出范围
			...
			switch(Language)
				...
				case ENGLISH:
						Printf24("Data exceeds limit!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
								break;
				case PORTUGAL: //  葡萄牙
						Printf16("Os dados acima do limite",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
								break;
				case FRENCH:	//法语
						Printf24("FORMAT EXCES LIMITE!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
								break;
				}
			...
			//----保存数据----------------
			if(Knife_order<=OnePro_head.pro_knifeSUM)  //不是 最后一刀 000.00
				{
				Knife_order++;
								  ...
								  if(Knife_order<=OnePro_head.pro_knifeSUM)
									 {
									  ...
									  if(Knife_order>1)
										{
										OnePro_head.pro_knifeSUM=Knife_order-1;
										}
									  else
										{
...
switch (keycode)
									...
									switch(Language)
										...
										case ENGLISH:
											Printf24("Invalid Mark!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
												break;
										case PORTUGAL: //  葡萄牙
											Printf16("Este sinalizador é definido ",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,1,0xffff);
											Printf16("				     definido inválido",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
											Printf16("				    			  lido",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
												break;
										case FRENCH:	//法语
											Printf24("MARQUAGE NON VALABLE!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
												break;
										}
									}
	...
	case ONE_KEY_CODE+OPTION_KEY_CODE:   //    ***************F功能按键+ “-/除”
			...
			if(WorkStart_flag==1)  
				...
				break;	
			//*****************
	case ONE_KEY_CODE+DENGJU_KEY_CODE:  //F+向上箭头		----左
			LongExec_flag=1;
			KeyExec(LEFT_KEY_CODE);
				break;
	case ONE_KEY_CODE+DENGFENG_KEY_CODE:  //F+向下箭头		----右
			LongExec_flag=1;
			KeyExec(RIGHT_KEY_CODE);
			break;
	 }
}
	...
	case	0xffff0009:  //     EQU_TOUCH_CODE       结果 =  
		if(WorkStart_flag==1)
			...
			if(Run_flag==0)
				...
				if(InputTouch_flag==1)
					{
					if((WorkMode == TEACH_MODE)||(WorkMode==AUTO_MODE))	//=======示教 模式========  运行到当前 值
						{
				...
				if(InputTouch_flag==0)
					...
					switch(WorkMode)
						...
						case PROG_LABEL_MODE://已 打开   标签********************
							...
							if(LabelTime_flag==0)
								{//废边模式
								label_valid_data=Total_size*Label_size*Waste_size;
								}
							else
								{
								label_valid_data=Total_size*Label_size*LabelTime;
								}
							if(label_valid_data!=0)//设定有效
								Label_function();          
							// }
							PromptNum_dis();//输入范围
							PromptDis_flag=1;
						...
						case PROG_PRO_MODE:	//已在 程序选择画面**************	
							WorkMode=PROG_PRO_MESS_MODE;//--------------显示程序详细 信息
					...
					switch(Language)
						...
						case PORTUGAL:	//葡萄牙
							...
							ASCII_Printf("ca",ASCII24_MODE,12,24,PUTAO_ZK24,x+12*23,y,0,back_color);
								break;
					    case FRENCH:	//法语
							Printf24("APPUYER CROIX ROUGE SUR ECRAN!",x,y,0,1,back_color);
								break;
						}
					TouchCalcu_mode=0;
		    ...
		    if(Run_flag==0)
				...
				if(InputTouch_flag==0)
					...
					switch(WorkMode)
						...
						case PROG_PRO_MODE:		//F3 程序模式
						case TEACH_PRO_MODE:	//程序列表 模式
							DeleteOneProg(cursor_pro, prog_order);		//F4--删除一个程序
							break;
						...
						case MANUAL_MODE: 
							...
							else if(MotorType>=MOTOR_AC_SERVO)//电子手轮 开关
								{
								ElectricWheelOn_flag=!ElectricWheelOn_flag;
				...
				case SELF_CUT_MODE://自刀主画面下
					if(num_cou>0)
						...
						if(CheckInputNums()==1)// 输入 有效
							...
							if(Knife_order<=OnePro_head.pro_knifeSUM)
								{
								if(OnePro_head.pro_knifeSUM<MAIX_Knife)
									{
									...
									switch(Language)
										...
										case PORTUGAL:	//葡萄牙
											...
											break;
										case FRENCH:	//法语
											Printf16("DONNEES PLEINES,NE PAS INSERER!",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
											break;
										}
									}
								}
								...
								switch(Language)
									...
									case PORTUGAL:	//葡萄牙
										...
										ASCII_Printf("a",ASCII24_MODE,12,24,PUTAO_ZK24,REMIND_WINDOW_X+12*13,REMIND_WINDOW_Y+5,RED_COLOR,0xffff);
											break;	
									case FRENCH:	//法语
										Printf24("ENTRER ET CONFIRMER!",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
											break;
									}
								}
							}
						...
						case 3://语言
							...
							switch(Language)
								...
								case CHN_HAN:
									...
									break;
								case ENGLISH:	
									x=PARA_START_X1,y=PARA_START_Y,gap=PARA_ROW_GAP;back_color=TEXT_TAB_BACK_COLOR;
								...
								case FRENCH:	//法语
									...
									x=PROG_X,y=PROG_Y;back_color=STEP_BACK_COLOR;
									GLCD_SetWindow_Fill( x, y, x+100, y+24, back_color,back_color);//clear text win
									Printf16("PROGRAMME",x,y,BLACK_COLOR,1,back_color);
									x=STEP_X;//
									GLCD_SetWindow_Fill( x, y, x+100, y+24, back_color,back_color);//clear text win
									Printf16("PROCEDURE",x,y,BLACK_COLOR,1,back_color);
					...
					if(Knife_order<=OnePro_head.pro_knifeSUM)
						...
						if(OnePro_head.pro_knifeSUM>1)
							...
							switch(Language)
								{
								case CHN_HAN:
									Printf24("长按超过5秒，删除以下全部!",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
										break;
								case ENGLISH:
									Printf24("Long-press 5 sec,deleate all!",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
										break;	
								case PORTUGAL:	//葡萄牙
									Printf16("Press?o longa, todos os excluídos",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);	
									ASCII_Printf("a",ASCII16_MODE,8,16,PUTAO_ZK16,REMIND_WINDOW_X+8*5,REMIND_WINDOW_Y+5,RED_COLOR,0xffff);
											break;	
								case FRENCH:	//法语
									Printf16("APPUYER 5 SECONDES POUR TOUT SUPPRIMER!",REMIND_WINDOW_X,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
										break;	
								}
      ...
      qianjinflag=0;
        ...
        if(WorkStart_flag==1)  
			...
			switch(MotorType)
				...
				case MOTOR_AC_SERVO:
				case MOTOR_AC_SERVO670:
					if(RunEnb_flag==1)// if(StartCheck_mode==2)
						{
						if(Board_TestMod==0)       //-----
							{
							if(PassCodeEn_flag==0)
			 ...
			 switch(WorkMode)
					...
					case PROG_PRO_MODE:	//程序选择*****************************	   
						if(num_cou>0)
							...
							{
							 if((prog_order>0)&&(prog_order<=MAIX_PROG))// 有效  更新一个程序，并显示--------------------
								{
								if(InputTouch_flag==0)    //非 触摸计算器 下
									{
									cursor_pro=0;
												...
												switch(Language)
													...
													case PORTUGAL:	//葡萄牙
														Printf16("Os dados acima do limite",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
															break;
													case FRENCH:	//法语
														Printf24("DONNEES EXCES LIMITE!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
															break;
													}
												if(InputTouch_flag==1)   //触摸下
													{
							...
							switch(Language)
								...
								case CHN_HAN:
										Printf24("手动模式下,请按运行键!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
											break;
								case ENGLISH:
										Printf24("In this mode,press RUN key!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
											break;
								case PORTUGAL:	//葡萄牙
										Printf24("Pressione a tecla Run",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
											break;
								case FRENCH:	//法语
										Printf16("EN MODE MANUEL, APPUYER SUR 'EXECUTER' !",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);
											break;
								}
							}
											...
											switch(Language)
												...
												case PORTUGAL:	//葡萄牙
															Printf24("A senha está correta! bem-vindo    ",PASSCODE_X,PASSCODE_Y,RED_COLOR,1,0xffff);
																break;
												case FRENCH:	//法语
															Printf24("MOT DE PASSE CORRECT,BIENVENUE!",PASSCODE_X,PASSCODE_Y,RED_COLOR,1,0xffff);
												}
											if(RunEnb_flag==1)
												{
												MoveSpeed=2;
		  ...
		  if(Run_flag==0)
			 ...
			 switch(WorkMode)
				...
				case TEACH_PRO_MODE:    //程序选择----------------------
					if(InputTouch_flag==0)  //	 非 触摸计算器 下
						{
						  if(prog_order>=MAIX_PROG)//返回第一个 pro
							  {
							  // Pro_num111=1;
							  prog_order=1;
					...
					if(InputTouch_flag==0)  //	 非 触摸计算器 下
						{
						  if(prog_order<2)//返回最后 pro
							  {
							  prog_order=MAIX_PROG;
							  CursorPro_dis(cursor_pro,0);//光标
							  cursor_pro=MAIX_PROG%PROGS_1PAGE;
							  if(cursor_pro<1)
...
if((Pro_data.pro_sum<1)||(Pro_data.pro_sum>MAIX_PROG))//程序数 无效，则初始化
  {
  Pro_data.pro_num=1;//当前程序 号1
...
while(line<PROGS_1PAGE)
	...
	if(p_order<=MAIX_PROG)
		...
		{
		// x=TableDis_startcol;
		// y=RIGHT_ARROW_Y+(line%PAGE_KNIFE)*PROTEXT_HIGH;
	    // GLCD_SetWindow_Fill( x, y, x+243, y+PROTEXT_HIGH, back_color, back_color);//尺寸输入
		}
...
while(row<PAGE_KNIFE)
   ...
   if(start_order<=MAIX_Knife)
      ...
      {
	  x1=RIGHT_ARROW_X;
      y1=RIGHT_ARROW_Y+row*SIZETEXT_HIGH;
	  y2=y1+SIZETEXT_HIGH;
	  if(InputTouch_flag==0)  //	 非 触摸计算器 下
		  x2=RED_KNIFE_X+25;
	  else
	 ...
	 switch(WorkMode)
		...
		case TEACH_LABEL_MODE:// 示教 标签	
				if(PorgSet_sel<1)	//========总长
					...
					if((InputSize<1)||(InputSize>Size_MAX_num))//超出范围
						{
						PromptDis_flag=0;
						GLCD_SetWindow_Fill( REMIND_WINDOW_X, REMIND_WINDOW_Y, REMIND_WINDOW_X+REMIND_LENGTH, REMIND_WINDOW_Y+30, 0xfffff, 0xffff);//提示框
						switch(Language)
...
switch(sel)
	...
	case 1:	//标签长
		...
		if(flag==1)
			...
			break;
	case 2://废边	次数
		ProgSet_dis(nums_temp, WASTE_SIZE_X,  WASTE_SIZE_Y,flag);
		if(flag==1)
			{
			PromptDis_flag=0;
			GLCD_SetWindow_Fill( REMIND_WINDOW_X, REMIND_WINDOW_Y, REMIND_WINDOW_X+REMIND_LENGTH, REMIND_WINDOW_Y+30, 0xfffff, 0xffff);//提示框
...
if(order>=OnePro_head.pro_knifeSUM)
   ...
   if(order<=MAIX_Knife)
	  ...
	  {
	  OnePro_head.pro_knifeSUM=MAIX_Knife;
	  }
   }
SavePro_headInfo(Pro_data.pro_num, (pInt8U)&OnePro_head);//刀数 保存
// StepNum_dis(OnePro_head.pro_knifeSUM,Knife_order);//步骤更新
if(LabelTime_flag==0)
	...
	{
	if((order+1)>=OnePro_head.pro_knifeSUM)
		{
		Knife_order=OnePro_head.pro_knifeSUM;
		}
	else
...
switch(PorgSet_sel)
	{
		case 0://总长
				if(CheckInputNums()==1)//数据有效
					...
					if(Total_size!=InputSize)
						...
						if(LabelTime_flag==0)
							{//废边模式
							nums_temp=Label_size*Waste_size;
							}
						else
							{
							nums_temp=Label_size*LabelTime;
							}
						if(nums_temp>=0) //数据 已经输入完毕
							{
		...
		case 1://标签
				if(CheckInputNums()==1)//数据有效
					...
					if(Label_size!=InputSize)
						...
						if(LabelTime_flag==0)
							{//废边模式
							nums_temp=Total_size*Waste_size;
							}
						else
							{
							nums_temp=Total_size*LabelTime;
							}
						if(nums_temp>0)
							{
							if(InputTouch_flag==1)   //触摸下  计算器 弹出框消失
		...
		case 2://
				if(CheckInputNums()==1)//数据有效
					...
					if(nums_temp!=InputSize)
						{
						nums_temp=InputSize;//
						if(LabelTime_flag==0)
							{//废边模式
							Waste_size=nums_temp;
							}
						else
							{
							LabelTime=nums_temp;
...
F2DivideIconUp_dis(F2_X,F2_Y);
F3up_dis(F3_X,F3_Y);
F4PushIconUp_dis(F4_X,F4_Y);
...
KnifeUp_dis(KNIFE_X,KNIFE_Y);//刀上
PressUp_dis(PRESS_X,PRESS_Y);//压上
...
{
Int16U p,	l,t,flag;
volatile Int32U date[MAIX_PROG+2],temp;
pInt8U pdata;
PROCESS_HEAD pro_Head;
...
/****	
for(l=1;l<MAIX_PROG;l++)
	{
	for(t=l+1;t<=MAIX_PROG;t++)		//选择法
		{
		if(date[t]>date[l])
			{
			temp=date[t];
	...
	//					break;
		case PORTUGAL:	//葡萄牙
				Printf24("Apagando, 6 segundo",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
		case FRENCH:	//法语
				Printf16("SUPPRESSION EN COURs,ATTENDRE 6 SECONDES!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
		}
while(c<=MAIX_PROG)	//清除信息头并保存 head_info=0
	{
...
FlashChip_Dis;
if(MotorType>MOTOR_AC_SERVO670)
...
void ReadOutLimitValid_flag(void)
{
Int32U add;
// pInt8U pdata;
add=ORIGINAL_25PEADD+116;
FastRead_Datas_Start(add);
OutFrontLimNoValid_flag=ReadFlash_Datas();
OutMiddLimNoValid_flag=ReadFlash_Datas();
OutBackLimNoValid_flag=ReadFlash_Datas();
FlashChip_Dis;
if(OutFrontLimNoValid_flag>1)
	OutFrontLimNoValid_flag=0;
if(OutMiddLimNoValid_flag>1)
	OutMiddLimNoValid_flag=0;
if(OutBackLimNoValid_flag>1)
	OutBackLimNoValid_flag=0;
}
void SaveOutLimitValid_flag(void)
{
Int32U add;
pInt8U pdata;
add=ORIGINAL_25PEADD+116;
pdata=(pInt8U)&OutFrontLimNoValid_flag;
Write_25pe_data(pdata, add, 1);
add++;
pdata=(pInt8U)&OutMiddLimNoValid_flag;
Write_25pe_data(pdata, add, 1);
add++;
pdata=(pInt8U)&OutBackLimNoValid_flag;
Write_25pe_data(pdata, add, 1);
}
...
if((size_data>=Size_MIN_num)&&(size_data<=Size_MAX_num))//数据有效
	...
	if(size_data!=CurrentSize)
		...
		if(Unit==UNIT_MM)//mm
			...
			/////////////////////////////前10mm 结束///推纸////////////////////////////
		if((OneKnife_data.k_flag==PUSH_FLAG)&&(Knife_order<OnePro_head.pro_knifeSUM))//推纸 位置提前10mm 结束
			{
	...
	if(last_num<42)  // 输入为 数字
		...
		if(CalcSign_order<1)  //-----------首个运算符  只显示
			...
			if(CalcResultDis_flag==1)   //   已得到  运算结果        
				{
				CalcResultDis_flag=0;  //结果显示 标志
				//CalcResult_Text[CalcInput_cou++]=sign;
			...
			if((size_data>=Size_MIN_num)&&(size_data<=Size_MAX_num))  //当前尺寸  有效
				...
				if(size_data>AutoPush_num)
					...
					if(Unit==UNIT_MM)//mm
						...
						/////////////////////////////前10mm 结束///推纸////////////////////////////
					if((OneKnife_data.k_flag==PUSH_FLAG)&&(Knife_order<OnePro_head.pro_knifeSUM))// 位置提前10mm 结束
						{
						if(CurrentSize>TargetSize)
							{
							if(CurrentSize-TargetSize>=1000)
...
switch(Language)
	...
	case PORTUGAL:	//葡萄牙
		...
		Printf24("Cancelar",DEL_ALL_CANCEL_X,DEL_ALL_CANCEL_Y,BLACK_COLOR,1,fill_color);
			break;
	case FRENCH:	//法语
		Printf24("SUPPRIMER TOUTES DONNEES?",DEL_ALL_HINT_X,DEL_ALL_HINT_Y,BLACK_COLOR,0,fill_color);
		Printf24("OK",DEL_ALL_OK_X,DEL_ALL_OK_Y,BLACK_COLOR,1,fill_color);
		Printf24("ANNULER",DEL_ALL_CANCEL_X,DEL_ALL_CANCEL_Y,BLACK_COLOR,1,fill_color);
			break;
	}
switch(WorkMode)
	{
	case PROG_PRO_MODE:
			WorkMode=PROG_DEL_ALL_PRO_MODE;
			break;
	case TEACH_PRO_MODE:
...
switch(Language)
	...
	case ENGLISH:	
		...
		break;
	case PORTUGAL:	//葡萄牙
		Printf16("Digite o texto, muda o nome",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
		break;
	case FRENCH:	//法语
		Printf24("MODIFIER NOM PROGRAMME",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
		break;
	}
}
