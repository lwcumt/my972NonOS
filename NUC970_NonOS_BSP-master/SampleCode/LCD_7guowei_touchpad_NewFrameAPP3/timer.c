			...
			if(SelfLockEn_flag==1)
				...
				if(StopDelay_flag==1)
					...
					if(TargetSize<CurrentSize)
						{
						temp=CurrentSize-TargetSize;
						if(temp>(MARGIN_SIZE-10))  //继续  低速  前进
							{
							//if(Forward_flag==0)
								//{
								MoveForward();//前进
						...
						{
						temp=TargetSize-CurrentSize;
						if(temp>(MARGIN_SIZE-10))  //继续  低速  后退
							{
							//if(Backward_flag==0)
							//	{
								MoveBack();  //后退
  ...
  if(houtui==1)
	...
	if(WorkStart_flag==1)
		...
		if((GoFrontEn_flag==1)||(GoBackEn_flag==1))
			...
			if(temp<=10000)//100mm
				outpw(REG_ETMR1_CMPR,(0xffff- Table500[step_add]));	//speed =500
			else

				outpw(REG_ETMR1_CMPR,(0xffff- waitTemp[step_add]));	//T1MR1 =(0xffff- waitTemp[step_add]);
			#else
				outpw(REG_ETMR1_CMPR,(0xffff- waitTemp[step_add]));
			#endif
			}
		}
	else
