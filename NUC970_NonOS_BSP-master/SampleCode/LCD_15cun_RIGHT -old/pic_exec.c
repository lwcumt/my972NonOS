...
Printf16("Copyright(c)lwcumt-2019-1,All Rights Reserved.",10,10,RED_COLOR,0,0xffff);
...
//--------------，进退.......
x1=BACK_PAGE_X,y1=BACK_PAGE_Y;
x2=FRONT_PAGE_X+JOG_BUTT_WIDE,y2=FRONT_PAGE_Y+JOG_BUTT_HIGH;
...
x=F3_PRO_CAPTION_X+30,y=F3_PRO_CAPTION_Y;
// CurrentArea_save2(MIDDLE_PAGE_X , MIDDLE_PAGE_Y,  M_PAGE_WIDE, M_PAGE_HIGH);
MiddlePage_dis();//中间活页
//主模式......
...
switch(Language)
	{
	case CHN_HAN:
		Printf16("序号        名称",x,y,line_color,0,back_color);	x+=400;
		Printf16("步骤        修改日期",x,y,line_color,0,back_color);
			break;
	case ENGLISH:	
		Printf16("Order       Name",x,y,line_color,0,back_color);	x+=400;
...
switch(Language)
		{
		case CHN_HAN:
			Printf16("F1-气垫,F2-压纸,F3-理纸,F4-推纸,F5-刀",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
				break;
		case ENGLISH:	
		case PORTUGAL:	//葡萄牙
		case SPANISH: //西班牙
			Printf16("F1-AIR,F2-PRE,F3-SORT,F4-PUSH,F5-KNIFE",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
				break;
		case INDONSIA:	//印尼
...
switch(page)
	...
	case 0:
		...
		switch(Language)
			...
			default:
					Printf24("air",x,y+20,BLACK_COLOR,0,0xffff);
					break;
			}
		// // *********************第七行
		// // x=start_x+gap;y+=70;
...
x1=RED_KNIFE_PIC_X;
y1=RED_KNIFE_PIC_Y+row*Kf_Line_HIGH;
x2=TABLE_END_COL;
y2=y1+FLAG_PIC_HIGH;
...
switch(motor)
	...
	case MOTOR_AC_SERVO:	 		//交流伺服	
		...
		switch(Language)
			{
			case CHN_HAN:
				Printf24("4------交流伺服正向",x,y,F_color,1,B_color);
				break;
			case ENGLISH:
			case INDONSIA:	//印尼
			case PORTUGAL:	//葡萄牙
	...
	case MOTOR_AC_SERVO_REV:	 		//交流伺服	
		...
		switch(Language)
			{
			case CHN_HAN:
				Printf24("5------交流伺服反向",x,y,F_color,1,B_color);
				break;
			case ENGLISH:
			case INDONSIA:	//印尼
			case PORTUGAL:	//葡萄牙
...
Num[i]=0;
x=CALCUL_TEXT_END-(8*i),y=CALCUL_TEXT_Y+5;
Printf16(Num,x,y,num_color,1,0xffff);	
}
...
Num[i]=0;
x=CALCUL_TEXT_END-(8*i),y=CALCUL_RESULT_Y+5;
Printf16(Num,x,y,num_color,1,0xffff);	
}
