...
while(1)
   ...
   if(Ms_Flag!=0)
	 ...
	 //************以下日期时间显示
		...
		FlashSignButton();//flash201909 F5++--
		//*************
		switch(WorkMode)
			{
			...
			if(senor==1)    //1   原点正常
				...
				if(OriginalError_flag==0)
						...
						switch(Language)
							{
							case CHN_HAN:
									Printf24("推纸器在后!",START_PROMPT_X1,START_PROMPT_Y1+5,RED_COLOR,1,0xffff);
									break;
							case ENGLISH:
									Printf24("Original fault!",START_PROMPT_X1,START_PROMPT_Y1+5,RED_COLOR,1,0xffff);
									break;
