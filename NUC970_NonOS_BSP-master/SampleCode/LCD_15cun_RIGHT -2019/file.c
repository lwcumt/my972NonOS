...
pSDbuf=(unsigned char *)NF_SECTOR_SDRAM_BUF+2048;
...
for(i=FILE_DIR_START_SECTOR;i<FILE_FILE_START_SECTOR;i++)
	...
	if(foldertemp->FileType==TYPE_DIR)
		...
		rsector=NF_SECTOR_SDRAM_BUF+2048+256;//跳开0页
		for(x=0;x<foldertemp->FileNum;x++)
			{
			pFiles=rsector;

			if(pFiles->FileType==TYPE_FILE)
 				{
				File_Debug("文件名：",0);
				File_Debug(pFiles->FileName,0);
				File_Debug("   ",0);

				File_Debug("长度：",0);
				sprintf(ascbuf,"%d",pFiles->FileLength);
				File_Debug(ascbuf,1);	
				}
			else
				{
...
while(1)
	{
	if(Sector_Info.BadSector[x]!=SECTOR_OK)
		{
		x++;
...
File_Debug("File Information block is......",0);
sprintf(ascbuf,"%d",x);
File_Debug(ascbuf,1);	


unsigned char *sd=(unsigned char*)&Sector_Info;
...
i=Nand_Write_1Page(sd,x*NF_BLOCK_PAGES+2); //写入块信息头	
sd+=NF_PAGE_BYTES;
i=Nand_Write_1Page(sd,x*NF_BLOCK_PAGES+3); //写入块信息头	
sd+=NF_PAGE_BYTES;
i=Nand_Write_1Page(sd,x*NF_BLOCK_PAGES+4); //写入块信息头	
sd+=NF_PAGE_BYTES;
i=Nand_Write_1Page(sd,x*NF_BLOCK_PAGES+5); //写入块信息头	
return 0;	
}
...
while(1)
	...
	if((Sector_Info.BadSector[x]!=SECTOR_OK))//||(Sector_Info.BadSector[x]!=SECTOR_BAD))
		{
		x++;

		if(x>FILE_INFO_START_SECTOR+MAX_BAD_SECTOR_COUNT)
			{
			File_Debug("File Information block not find......",1);
			return -1;//坏块超过记录数量
			}
		}
	else
...
for(x=0;x<NAND_Blocks;x++)
	{
	if(Sector_Info.BadSector[x]==SECTOR_BAD)
				{
				badblock++;
				}
...
int n;
		...
		File_Debug("Need Free Block as:",0);		
		sprintf(ascbuf,"%d",flen);
		File_Debug(ascbuf,1);	

for(startsector=FILE_FILE_START_SECTOR;startsector<(NAND_Blocks);startsector++)
	{
	if(startsector>=(NAND_Blocks-flen))
		{
		File_Debug("No Free Block......\r\n:",1);		
		File_Debug("Cur secher Block Add is:",0);		
...
if(File_Find((unsigned char *)nametemp)==1)
	...
	nedd_sector++;
	 ...
	 for(i=0;i<nedd_sector;i++)
			...
			Nand_EraseBlock(i+nFiles_Temp.FileSectorAddr);//*NF_BLOCK_BYTES)>>11);

	 		File_Debug("Old usered block is :",0);
			sprintf(ascbuf,"%d",i+nFiles_Temp.FileSectorAddr);
			File_Debug(ascbuf,1);
	   ...
	   //获取空间
		...
		if(sector>0)
				...
				for(n=sector;n<nedd_sector+sector;n++)
					...
					Nand_EraseBlock(n);//*NF_BLOCK_BYTES)>>11);
	 				...
	 				File_Debug("New usered block is :",0);
					sprintf(ascbuf,"%d",n);
					File_Debug(ascbuf,1);
		...
		memcpy(nFiles.FileName,nFiles_Temp.FileName,128);
		...
		File_Debug("",1);
		File_Debug("Wrtie File Name to Sdram Buffer......:  ",0);

		File_Debug(nFiles.FileName,1);

        Nand_WritePage_SDbuf(FileNum,FILE_INFO_LENGTH,fnametemp);
		Nand_WriteBlock(File_Folder_Temp.StartSectorAddr,(unsigned char *)NF_SECTOR_SDRAM_BUF);	

		File_Debug("Pointer Filelength is...: ",0);
		sprintf(ascbuf,"%d",nFiles.FileLength);
		File_Debug(ascbuf,1);
		...
		File_Debug("This Dir File total Num is...: ",0);
	...
	if(startsector!=0)
		...
		for(filenum=0;filenum<8*63;filenum++)
				...
				if(strcmp(fname,pFiles->FileName)==0)
					{
					//拷贝到文件信息暂存
					memcpy((char*)&nFiles_Temp,(uint8_t*)pFiles,256);
