...
if((WorkMode==MANUAL_MODE)||(WorkMode==TEACH_MODE))  // 手动|示教 模式	默认当前尺寸
	...
	if(Knife_order>1)
		{
		GetOneKnife_datas(Pro_data.pro_num, Knife_order-1, (pInt8U)&Knife_datas);//取出前一个  数据
...
if((Knife_order<=MAX_Knife)&&(Knife_order>1))	//刀数据 未满
	...
	if(InputSize>0)
		...
		GetOneKnife_datas(Pro_data.pro_num, Knife_order-1, pKnife_data);// 读取前刀 尺寸
...
switch(keycode)
	{
	case IO_TEST_TOUCH_CODE:   // IO key++201909
			if(WorkStart_flag==1)
				{
				if(Run_flag==0)  //进入 -----------IO模式	
					{
					switch(WorkMode)
						{	
						case AUTO_MODE://自动主画面下
							WorkMode=AUTO_IO_MODE;Flg_chg=1;
								break;	
						case MANUAL_MODE:// 手动 模式
							WorkMode=MANUAL_IO_MODE;Flg_chg=1;	// 
								break;	
						case TEACH_MODE://示教 模式
							WorkMode=TEACH_IO_MODE;Flg_chg=1;
								break;
						case SELF_CUT_MODE:
							WorkMode=SELF_IO_MODE;Flg_chg=1;
								break;
						}
					if(Flg_chg!=0)	
						{
				...
				break;
	case PATAMETER_TOUCH_CODE:   // 参数key++201909
			if(WorkStart_flag==1)
				{
				if(Run_flag==0)  //进入 -----------参数模式	
					{
					switch(WorkMode)
						{	
						case AUTO_MODE://自动主画面下
							WorkMode=AUTO_SET_MODE;Flg_chg=1;
								break;	
						case MANUAL_MODE:// 手动 模式
							WorkMode=MANUAL_SET_MODE;Flg_chg=1;	// 
								break;	
						case TEACH_MODE://示教 模式
							WorkMode=TEACH_SET_MODE;Flg_chg=1;
								break;
						case SELF_CUT_MODE:
							WorkMode=SELF_SET_MODE;Flg_chg=1;
								break;
						}
					if(Flg_chg!=0)	
						{
				...
				break;
	case FIND_TOUCH_CODE:   // 查找key++201909
			if(WorkStart_flag==1)
				{
				if(Run_flag==0)  //
					{
					switch(WorkMode)
						{	
						case AUTO_MODE://自动主画面下
							WorkMode=AUTO_FIND_MODE;Flg_chg=1;
								break;	
						case MANUAL_MODE:// 手动 模式
							WorkMode=MANUAL_FIND_MODE;Flg_chg=1;	// 进入 -----------选择程序模式(查找模式)	
								break;	
						case TEACH_MODE://示教 模式
							WorkMode=TEACH_FIND_MODE;Flg_chg=1;
								break;
						case SELF_CUT_MODE:
							WorkMode=SELF_FIND_MODE;Flg_chg=1;
								break;
						}
					if(Flg_chg!=0)	
						{
	...
	case SHIFT_0_KEY_CODE:	//shift+0------ 气垫标记
			if(WorkStart_flag==1)
			  ...
			  if(Run_flag==0)
				 ...
				 switch(WorkMode)
					...
					case AUTO_MODE://自动--  主画面下
						if(FlagTouRow_num>0)//触摸key
							{
							if((cursor_row+1)!=FlagTouRow_num)//触摸点==光标处
								{	//****1****行跳转 选择20190422
								SelectKnifeCursor_dis(FlagTouRow_num-1);	
								}	
								//******2******
			 			 ...
			 			 break;		
					case AUTO_FLAG_MODE:	//其他标记wind 模式	 
						AllFlagSetExec(AIR_FLAG); //设定 气垫标记
						break;
					}
				 }
			  ...
			  break;		
	case SHIFT_1_KEY_CODE:	//shift+1------推纸标记 
			if(WorkStart_flag==1)
			  {
			  if(Run_flag==0)
				 {
	...
	case SHIFT_2_KEY_CODE:	//shift+2------ 理纸标记
			if(WorkStart_flag==1)
			  ...
			  break;	
	case SHIFT_3_KEY_CODE:	//shift+3------ 压纸标记
			if(WorkStart_flag==1)
			  {
			  if(Run_flag==0)
				 {
				...
				switch(WorkMode)
					...
					case AUTO_MODE://自动--  主画面下
						// if(MotorType>=MOTOR_AC_SERVO)
							...
							// }
						break;
					case AUTO_FLAG_MODE:	//标记 模式
						ClrCurrentFlag_Exec();	//清除当前标记
						break;
					}
				 }	
			  }
	...
	case SHIFT_8_KEY_CODE:	//shift+8------ 密码 模式
			if(WorkStart_flag==1)
			  ...
			  if(Run_flag==0)
					...
					switch(WorkMode)
						...
						case MANUAL_MODE: //手动模式
							if(Sign_flag==1)
							 ...
							 ACServoAlarmExec_flag=0;
							 break;
						case AUTO_MODE://自动主画面下	   --------其他标记位wind
							if(FlagTouRow_num>0)//触摸key
								{
								if((cursor_row+1)!=FlagTouRow_num)//触摸点==光标处
									{	//****1****行跳转 选择20190422
									...
									//******2******
								Set_AllFlagWind_In(0);  //其他标记位wind
								FlagTouRow_num=0;
				 				}
							else		//键盘key
								{
								Set_AllFlagWind_In(0);  //
								}
							 break;
						}
				 }
				...
				break;
	case LINE_SEL_KEY_CODE://行触摸选择	  20190422
			if(WorkStart_flag==1)
				 {
				 if(Run_flag==0)
					{
					switch(WorkMode)
						...
						//case TEACH_MODE://示教 模式
							SelectKnifeCursor_dis(FlagTouRow_num-1);	//行跳转
							FlagTouRow_num=0;
							break;
						case AUTO_FIND_MODE://
						...
						case SELF_FIND_MODE://
							SelectProg_Exec( TouchLnie_num-1);
							TouchLnie_num=0;
							break;
						}
					}
	...
	case HELP_KEY_CODE:         //帮助----------
		if(WorkStart_flag==0)
			...
			PLC_SendCmd_RCS( 'X',  1);//tttttttttttttttttttt
			
			}
		else//----------------------------------------------------------------------------------------tttttttttttttttttttttttttttttttttttttttt
			{
			}
			break;
			/***
		...
		if(WorkStart_flag==1)
			...
			if(Run_flag==0)
				...
				if(CalcResultDis_flag==0)
					{
					if((WorkMode!=AUTO_BATCH_MODE)&&(WorkMode!=TEACH_BATCH_MODE))//非批量mode
						{
						if(num_cou>0)	//有输入
							{
							CalcGetValue_exec();
							if(WorkMode==MANUAL_MODE)
								KeyExec(UP_KEY_CODE);    ///////////运行按键*********
							}
						else		//无输入
							{
							switch(WorkMode)
				...
				switch(WorkMode)
					...
					case SELF_FIND_MODE://
						switch(WorkMode)
							{
							case AUTO_FIND_MODE://
								WorkMode=AUTO_FIND_NAME_MODE;//--------------显示程序详细 信息
									break;	
							case MANUAL_FIND_MODE://
								WorkMode=MANUAL_FIND_NAME_MODE;//--------------
									break;	
							case TEACH_FIND_MODE://
								WorkMode=TEACH_FIND_NAME_MODE;//--------------
									break;	
							case SELF_FIND_MODE://	
								WorkMode=SELF_FIND_NAME_MODE;//--------------
									break;	
							}
					...
					/**
					case MANUAL_MODE:  // 手动模式
						WorkMode=MANUAL_SET_MODE;	//MANUAL_TOOLS_MODE;//进入  手动----参数模式*****************
						F1_ToolsPage_dis(); //参数画面
						Para_sel=0;
						PromptDis_flag=0;
						ACServoAlarmExec_flag=0;
					...
					case MANUAL_SERVO_C_MODE:   //手动直流伺服 C 参数 
						...
						switch(WorkMode)
							{
							case AUTO_SET_MODE:
								WorkMode=AUTO_MODE;//返回    ---------  主模式
									break;	
							case TEACH_SET_MODE:
								WorkMode=TEACH_MODE;//返回    ---------  主模式
									break;	
							case SELF_SET_MODE:	
								WorkMode=SELF_CUT_MODE;//返回    ---------  主模式
									break;	
							default:
								WorkMode=MANUAL_MODE;//返回    ---------手动 主模式
									break;	
			...
			if(Run_flag==0)
				...
				// // if(ButtonTouch_mode==0)
					...
					switch(WorkMode)
						...
						case TEACH_FLAG_MODE:	//其他标记wind 模式	 
								Set_AllFlagWind_Exit(0);  //退出标记框
							 break;
						//****************自刀
						/****
						...
						case SELF_IO_MODE:  //io测试模式
							switch(WorkMode)// WorkMode=MANUAL_MODE;//返回    ---------手动 主模式
								{
								case AUTO_SET_MODE:
								case AUTO_IO_MODE:
									WorkMode=AUTO_MODE;//返回    ---------  主模式
		    ...
		    if(Run_flag==0)
			    {
					switch(WorkMode)
						...
						case PROG_MODE://编程 主画面下
							AllFlagSetExec(PUSH_FLAG); //设定 推纸标记  //F4 标记位	模式
							FirstKnife_num=0;
							...
							case 3://语言
								...
								switch(Language)
									...
									case INDONSIA:	//印尼
										...
										x=PROG_X,y=PROG_Y;back_color=STEP_BACK_COLOR;
										GLCD_SetWindow_Fill( x, y, x+100, y+24, back_color,back_color);//clear text win
										Printf24("Program",x,y,BLACK_COLOR,1,back_color);
										x=STEP_X,y=STEP_Y;//
										GLCD_SetWindow_Fill( x, y, x+100, y+24, back_color,back_color);//clear text win
										Printf24("Langkah",x,y,BLACK_COLOR,1,back_color);
										break;	
									case PORTUGAL:	//葡萄牙
										x=PARA_START_X1,y=PARA_START_Y,gap=PARA_ROW_GAP;//back_color=TEXT_TAB_BACK_COLOR;//0xef9f;
										...
										if(ButtonTouch_mode==0)  //	 非 触摸计算器 下
											...
											switch(MotorType)
												...
												case MOTOR_FREQ_3_SPD:
												case MOTOR_FREQ_3_SPD_RST:
													Printf24("antes:",x,y,BLACK_COLOR,1,back_color);
														break;
												// // case MOTOR_DC_SERVO:
												// // case MOTOR_DC670_SERVO:
													// // Printf24("Veloci-má  ",x,y,BLACK_COLOR,1,back_color);
										...
										x=PROG_X,y=PROG_Y;back_color=STEP_BACK_COLOR;
										GLCD_SetWindow_Fill( x, y, x+100, y+24, back_color,back_color);//clear text win
										Printf24("Programa",x,y,BLACK_COLOR,1,back_color);
										x=STEP_X,y=STEP_Y;//
										GLCD_SetWindow_Fill( x, y, x+100, y+24, back_color,back_color);//clear text win
										Printf24(" Passo ",x,y,BLACK_COLOR,1,back_color);
										break;	
									case SPANISH: //西班牙
										...
										x=STEP_X,y=STEP_Y;//
										GLCD_SetWindow_Fill( x, y, x+100, y+24, back_color,back_color);//clear text win
										Printf24(" Paso ",x,y,BLACK_COLOR,1,back_color);
										break;	
									}
								ParameterPage1_dis(Para_sel, 1);
					...
					case MANUAL_MODE:   //手动
							if(num_cou>0)	
								...
								if(num_cou<8)
									...
									// ClearCalculatorInput(); //清零计算器 输入	20191029
									}
								else          //已 输入数据  直接运行
									{
						...
						if(Sign_flag==0)
							...
							if(num_cou>0)
							  ...
							  if(CheckInputNums()==1)// 输入 有效
								...
								//ClrAllFlagsInit(&OneKnife_data);//---2019----------当前所有标记clr初始化
								 if(Knife_order<OnePro_head.pro_knifeSUM)// 不是最后一刀
									...
									{
									OneKnife_data.k_flag=NOAIR_FLAG|FLAG_FLAG;
									SaveOneKnife_datas(Pro_data.pro_num, Knife_order , pKnife_data);//保存一刀数据
									}
						...
						if(num_cou>0)
							...
							if(Knife_order<=OnePro_head.pro_knifeSUM)
							   ...
							   if(CheckInputNums()==1)
									...
									if(CalcInput_Text[0]=='-')
										F6_AutoSub_exec(InputSize);  //--
									if(WorkMode==AUTO_BATCH_MODE)
										WorkMode=AUTO_MODE;
									else
										WorkMode=TEACH_MODE;
									ClearCalculatorInput(); //清零计算器 输入
								...
								if(PramSel_Page<1)	//page 1
									...
									switch(Para_sel)
										...
										case 11://press time
											if(MotorType>=MOTOR_AC_SERVO)  //*************
												{
												if((input_temp>=1)&&(input_temp<=10))	//	
													{
													AutoPressWork_TimeSet=input_temp;
													SaveAutoPressWork_TimeSet();
													ParameterPage1_dis(Para_sel, 1);
													}
												}
												break;
...
//pInt8U pbuf;
KNIFE_DATA Knife_datas;
Int16U x1,y1,x2,y2,back_color=TEXT_TAB_BACK_COLOR,size_color;//0xef9f;
Int8U change_flag=0,first_knife_0flag=0;
...
while(row<PageKf_Size)
   ...
   if(start_order<=MAX_Knife)
      {
		if(start_order<=(OnePro_head.pro_knifeSUM+1))//数据+0 显示
			{
			GetOneKnife_datas(Pro_data.pro_num, start_order , (pInt8U)&Knife_datas);//取出一刀 数据	
			change_flag=ClrAllFlagsInit((KNIFE_DATA*)&Knife_datas);//---2019----------当前所有标记clr初始化
			...
			if(change_flag != 0)//更新标记位
				{
				SaveOneKnife_datas(Pro_data.pro_num, start_order, (pInt8U)&Knife_datas);//保存一刀数据	
				change_flag=0;
				}
			
			if(start_order==1)	//第一刀==0             20190404
				{
				if(Knife_datas.size==0)
					{
					first_knife_0flag=1;
			...
			if(row==cursor_row)  //光标处
				...
				//显示-----1-----------	201903  
			if(start_order>=2)  //    无数据只显示一刀   20190404
				{
				if(first_knife_0flag==0)
					{
					OneKnifeDataSel_dis(start_order, row,size_color);  //一刀数据 		
				// if((ButtonTouch_mode==0)||(ButtonTouch_mode==TOU_MODE_F4)) //	 非 触摸计算器 下
					// AllFlagPic_dis(row,Knife_datas.k_flag); //  标志  				显示-----2-----------	
					}
				else	//	0000不显示
					{
					x1=RIGHT_ARROW_X;
...
if(WorkStart_flag==0)// if((ButtonTouch_mode==0)||(ButtonTouch_mode==TOU_MODE_F4)) // 标记框模式
	...
	{
	if((WorkMode==AUTO_BATCH_MODE)||(WorkMode==TEACH_BATCH_MODE))//
		{
		Size=(pInt8U)&CalcInput_Text[1];
		num_bits=CalcInput_cou-1;	
		}
	else	//
		{
	...
	if(data>d_size)
	   {
		...
		if((data>=Size_MIN_num)&&(data<=Size_MAX_num))//数据有效
		  {
	      OneKnife_data.size=data;// SaveOneKnife_datas(order++ , pKnife_data);//保存
		  OneKnife_data.k_flag=NOAIR_FLAG|FLAG_FLAG;//标记初始待设定
		  for(j=0;j<5;j++)
			 {
			 *pbuf++=*(pKnife_data+j);
...
FlashChip_Dis;
// if(MotorType>MOTOR_AC_A6_SERVO)
if(MotorType>MOTOR_AC_SERVO_REV)
	MotorType=MOTOR_AC_SERVO;//MOTOR_FREQ_3_SPD_RST;	//  ------变频器3挡
}
...
if(PageKf_Size_flag>1)
	PageKf_Size_flag=0;
	PageKf_Size_flag=PAGE_7_LINE;//201909
}
...
Int8U n;
Int32U add;
add=ORIGINAL_25PEADD+138;
FastRead_Datas_Start(add);	
LuoJu_set=ReadFlash_Datas();
FlashChip_Dis;
if(LuoJu_set>1)
	LuoJu_set=0;//10mm
}
...
Int8U n;
Int32U add;
pdata=(pInt8U)&LuoJu_set;
add=ORIGINAL_25PEADD+138;
Write_25pe_data(pdata, add, 1);	
}

void ReadAutoPressWork_TimeSet(void)//autopress time
{
pInt8U pdata;	
Int8U n;
Int32U add;
add=ORIGINAL_25PEADD+136;	
FastRead_Datas_Start(add);	
AutoPressWork_TimeSet=ReadFlash_Datas();
FlashChip_Dis;
if(AutoPressWork_TimeSet>10)//10s
	AutoPressWork_TimeSet=3;
}
void SaveAutoPressWork_TimeSet(void)
{
pInt8U pdata;	
Int8U n;
Int32U add;
add=ORIGINAL_25PEADD+136;	
pdata=(pInt8U)&AutoPressWork_TimeSet;
Write_25pe_data(pdata, add, 1);	
}
//*******
...
if((AutoPushDistance>AUTO_PUSH_NO)&&(AutoPushDistance<=AUTO_PUSH_DISTAN))  //自动推纸 有效
	...
	if(AutoPushRun_flag==0)   //未运行到 自动推纸 距离
		...
		GetOneKnife_datas(Pro_data.pro_num, Knife_order, pKdata);//
		size_data=OneKnife_data.size;
		cur_flag=OneKnife_data.k_flag&0x0f;
		if(Knife_order>=OnePro_head.pro_knifeSUM)//20191122
			GetOneKnife_datas(Pro_data.pro_num, 1, pKdata);//
		else
			GetOneKnife_datas(Pro_data.pro_num, Knife_order+1, pKdata);//
		
		if((size_data<OneKnife_data.size)&&(cur_flag!=SORT_FLAG))  //当前<下一刀    !=理纸       
			{
...
BackWindDelete_dis(DEL_ALL_WIN_X,DEL_ALL_WIN_Y);
...
switch(WorkMode)
	...
	// case READ_PRO_MODE:
			...
			// break;	
	case AUTO_FIND_MODE:
			WorkMode=AUTO_FIND_DEL_ALL_MODE; 
			break;
	case MANUAL_FIND_MODE:		
			WorkMode=MANUAL_FIND_DEL_ALL_MODE;//删除所有pro
			break;
	case TEACH_FIND_MODE:
			WorkMode=TEACH_FIND_DEL_ALL_MODE;
			break;	
	case SELF_FIND_MODE:
			WorkMode=SELF_FIND_DEL_ALL_MODE;
			break;
		//-------------	
	case TEACH_MODE:
...
switch(WorkMode)
	...
	// case READ_DEL_ALL_PRO_MODE:
			...
			// break;
	case AUTO_FIND_DEL_ALL_MODE:
			WorkMode=AUTO_FIND_MODE;
			break;	
	case MANUAL_FIND_DEL_ALL_MODE:
			WorkMode=MANUAL_FIND_MODE;
			break;	
	case TEACH_FIND_DEL_ALL_MODE:
			WorkMode=TEACH_FIND_MODE;
			break;	
	case SELF_FIND_DEL_ALL_MODE:
			WorkMode=SELF_FIND_MODE;
			break;	
		//-------------
	...
	if(Run_flag==0)
		...
		// if(ButtonTouch_mode==0)//未有  弹出页
			...
			switch(WorkMode)
				...
				case AUTO_MODE:
				case TEACH_MODE:
						// WorkMode=AUTO_PLUS_MODE; 
						if(WorkMode==AUTO_MODE)
							WorkMode=AUTO_BATCH_MODE;  //F5++--
						else
							WorkMode=TEACH_BATCH_MODE;
						BatchSign_flag=0;
						F5_plus_dis(1);//  ++
						...
						if(MotorType<MOTOR_AC_SERVO)    //变频器--
							{
							 ...
							 // PulseDegreePage_dis();
							 // PromptDis_flag=0;// // ClrInput_function();//清零 输入框
		...
		// if(ButtonTouch_mode==0)//未有  弹出页
			...
			switch(WorkMode)
				...
				case SELF_FIND_MODE://
						...
						CurrentArea_recover2(MIDDLE_PAGE_X , MIDDLE_PAGE_Y,  M_PAGE_WIDE, M_PAGE_HIGH);
						SubModeIcon_dis(WorkMode);   //模式name
						if((Knife_order==1)&&(WorkMode!=MANUAL_MODE))// ----程序已被删除---------
							{
							//------------返回第一刀
							ClrInput_function();//清零 输入框 
							CursorKnife_dis(cursor_row, 0);//红色光标消失
...
if(ButtonTouch_mode==TOU_MODE_F4)  // 标记 弹出框
	Set_AllFlagWind_Exit(0);//(FlagTouRow_num-1);	//退出标记框  
//----------------------
		...
		switch(flag)
			...
			case PRESS_FLAG:    // 压纸
				if((OneKnife_data.k_flag&0x0f)!=flag) //设定 标志
					...
					break;
			case AIR_FLAG:      //气垫
				if((OneKnife_data.k_flag&0xf0)!=flag) //设定 标志
					{
					while(knife_begin<=knife_end)
						{
						*pbuf&=0x0f;
		...
		switch(flag)
			...
			case PRESS_FLAG:    // 压纸
				if((OneKnife_data.k_flag&0x0f)!=flag) //
					...
					{
					OneKnife_data.k_flag&=0xf0;// 清除 标志
					OneKnife_data.k_flag|=FLAG_FLAG;//标记初始待设定
					}
				renew_flag=1;
					break;
			case AIR_FLAG:      //气垫
				if((OneKnife_data.k_flag&0xf0)!=flag) //
					...
					OneKnife_data.k_flag|=NOAIR_FLAG;//标记初始待设定
					}
				renew_flag=1;
					break;
			case  KNIFE_FLAG:     //刀  标志	
...
void SelectKnifeCursor_dis(Int8U cursor)
{
volatile Int8U data_ok_flag=0,offset;
Int8S offset_s;
if(OnePro_head.pro_knifeSUM>1)//有数据
	{
	data_ok_flag = 1;
	}
else	//	只有一刀
	{
	pKnife_data=(pInt8U)&OneKnife_data;
	GetOneKnife_datas(Pro_data.pro_num, Knife_order, pKnife_data);// 读取 尺寸	
	if((OneKnife_data.size>=Size_MIN_num)&&(OneKnife_data.size<=Size_MAX_num))//数据有效
		{
		data_ok_flag = 1;
		}
	}
if(data_ok_flag==1)
	{
		/**-------1.无符号算法
	if(cursor>cursor_row)//当前光标 之后
		{
		offset =cursor-cursor_row;
		if((Knife_order+offset)<=(OnePro_head.pro_knifeSUM+1))//未超出 数据区
			{
			CursorKnife_dis(cursor_row, 0);//红色光标消失
			OneKnifeDataSel_dis(Knife_order, cursor_row,0);  //一刀数据 高亮显示消失
			///++++++++select
			Knife_order+=offset;
			cursor_row=cursor;
			CursorKnife_dis(cursor_row, 1);//红色光标显示
	...
	else if(cursor<cursor_row)		//当前光标 之前
		{
		offset =cursor_row-cursor;	
		CursorKnife_dis(cursor_row, 0);//红色光标消失
		OneKnifeDataSel_dis(Knife_order, cursor_row,0);  //一刀数据 高亮显示消失
		///++++++++select
		Knife_order-=offset;
	...
	if(cursor!=cursor_row)//当前光标 之后
		{
		offset_s=(Int8S)cursor-(Int8S)cursor_row;
		if((Knife_order+offset_s)<=MAX_Knife)
			{
			if((Knife_order+offset_s)<=(OnePro_head.pro_knifeSUM+1))//未超出 数据区
				{
				CursorKnife_dis(cursor_row, 0);//红色光标消失
				OneKnifeDataSel_dis(Knife_order, cursor_row,0);  //一刀数据 高亮显示消失
				///++++++++select
				Knife_order+=offset_s;
				cursor_row=cursor;
				CursorKnife_dis(cursor_row, 1);//红色光标显示
				OneKnifeDataSel_dis(Knife_order, cursor_row,RED_COLOR);  //一刀数据 高亮显示
				//-------
...
void ClrCurrentFlag_Exec(void)//清除当前标记
{
if(ButtonTouch_mode==TOU_MODE_F4)  // 标记 弹出框
	Set_AllFlagWind_Exit(0); //退出标记框 
pKnife_data=(pInt8U)&OneKnife_data;	
GetOneKnife_datas(Pro_data.pro_num, Knife_order, pKnife_data);
if((OneKnife_data.k_flag&0x0f)!=FLAG_FLAG)//已有标记
	{
	OneKnife_data.k_flag&=0xF0;
	OneKnife_data.k_flag|=FLAG_FLAG;//标记待设定（清除）
	//-------保存
	RenewModifyDate();	//更新时间信息-.-.-.-.-.-.-.-.-.-.-.-.-.-
	SaveOneKnife_datas(Pro_data.pro_num, Knife_order , pKnife_data);//保存一刀数据	
	AllFlagPic_dis(cursor_row,OneKnife_data.k_flag); //显示----
...
void SelectProg_Exec(Int8U cursor)
{
Int8S offset;
if(cursor!=cursor_pro)
	{
	CursorPro_dis(cursor_pro,0);//光标	
	OneProgHeadInfo_dis(cursor_pro ,  prog_order, 0);	//一条程序 显示0
	offset=(Int8S)(cursor-cursor_pro);
	prog_order=(Int16S)prog_order+offset;
	cursor_pro=cursor;
	CursorPro_dis(cursor_pro,1);//光标	
	OneProgHeadInfo_dis(cursor_pro ,  prog_order, 1);	//一条程序 显示
