...
Int8U flashbutt_mode;
Int16U flashbutt_time;
...
FileFolderRead("示教模式up.bmp",(unsigned char*)TEACH_MOD_UP_sdram);
FileFolderRead("自刀模式up.bmp",(unsigned char*)AUTO_CUT_MOD_UP_sdram);
//------------------------------main-mode down
...
FileFolderRead("示教模式down.bmp",(unsigned char*)TEACH_MOD_DOWN_sdram);
FileFolderRead("自刀模式down.bmp",(unsigned char*)AUTO_CUT_MOD_DOWN_sdram);
...
//----public mode
FileFolderRead("查看模式.bmp",(unsigned char*)FIND_MOD_UP_sdram);
FileFolderRead("参数模式.bmp",(unsigned char*)PARAMETER_sdram);
FileFolderRead("IO模式.bmp",(unsigned char*)IO_TEST_sdram);
//------------------------------F1,2,3...
...
FileFolderRead("F5批量.bmp",(unsigned char*)F5_ADD_UP_sdram);
...
//
FileFolderRead("delete_win.bmp",(unsigned char*)DEL_BCK_WIN_sdram);//del win
FileFolderRead("autoK_win.bmp",(unsigned char*)MOD_NOTE_WIN_sdram);//note win
}
...
Printf16("Copyright(c)lwcumt-201912,All Rights Reserved.",10,10,RED_COLOR,0,0xffff);
...
switch(mode)
	...
	case AUTO_BATCH_MODE:  //		
	case TEACH_BATCH_MODE:  //
		switch(Language)
			{
			case CHN_HAN:
				x+=12*3;	
				Printf24("批量加减模式",x,y,line_color,1,back_color);
					break;
			case INDONSIA:	//印尼
				// x+=12*2;		
			...
			case ENGLISH:	
				x+=12*3;	
				Printf24("++-- mode",x,y,line_color,1,back_color);
					break;
			}	
			break;
...
void TeachModeUp_dis(Int16U x,Int16U y)//示教按钮up
{
Bmp_Decode((Int32U)TEACH_MOD_UP_sdram, x, y, (pInt16U)SDRAM0_BASE_ADDR);	
}
void TeachModeDown_dis(Int16U x,Int16U y)//示教按钮
{
Bmp_Decode((Int32U)TEACH_MOD_DOWN_sdram, x, y, (pInt16U)SDRAM0_BASE_ADDR);	
}
void AutoCutMOdeUp_dis(Int16U x,Int16U y)//自刀按钮up
{
Bmp_Decode((Int32U)AUTO_CUT_MOD_UP_sdram, x, y, (pInt16U)SDRAM0_BASE_ADDR);	
}
void AutoCutMOdeDown_dis(Int16U x,Int16U y)//自刀按钮
{
Bmp_Decode((Int32U)AUTO_CUT_MOD_DOWN_sdram, x, y, (pInt16U)SDRAM0_BASE_ADDR);	
}
///quick  mode
...
void ParamaterIconUp_dis(Int16U x,Int16U y)//参数
{
Bmp_Decode((Int32U)PARAMETER_sdram, x, y, (pInt16U)SDRAM0_BASE_ADDR);
}
void IoTestButton_dis(Int16U x, Int16U y)	//io  test
{
Bmp_Decode((Int32U)IO_TEST_sdram, x, y, (pInt16U)SDRAM0_BASE_ADDR);
}
void ShortcutButton_dis(void)  //查找 +参数+ io  -------
{
FindIconUp_dis(FIND_BUTT_X,FIND_BUTT_Y);
ParamaterIconUp_dis(PARAMETER_BUTT_X,PARAMETER_BUTT_Y);
IoTestButton_dis(IO_TEST_BUTT_X,IO_TEST_BUTT_Y);
}
//------------------------------=====power ctrl==========
...
void IR_LINE_dis(Int16U x, Int16U y)//红外线xxxxxxxxx20191015
{
// Bmp_Decode((Int32U)IR_LINE_sdram, x, y, (pInt16U)SDRAM0_BASE_ADDR);
}
void IR_CUT_dis(Int16U x, Int16U y)//红外断开xxxxxxxxxx20191015
{
...
void BackWindDelete_dis(Int16U x, Int16U y)//del wind
{
Bmp_Decode((Int32U)DEL_BCK_WIN_sdram, x, y, (pInt16U)SDRAM0_BASE_ADDR);
}
void BackWindNote_dis(Int16U x, Int16U y)//note wind
{
Bmp_Decode((Int32U)MOD_NOTE_WIN_sdram, x, y, (pInt16U)SDRAM0_BASE_ADDR);
}
...
x2=x1+FICONBUTT_WIDE,y2=y1+FICONBUTT_GAP_V*6-10;
...
//--------------，运行.......
x1=RUN_BUTT_X,y1=RUN_BUTT_Y;
x2=STOP_X+RUN_BUTT_WIDE,y2=STOP_Y+RUN_BUTT_HIGH;
GLCD_SetWindow_Fill(x1,y1,x2,y2,back_color,back_color);  //，运行..
//--------clr shortcut-icon
x1=FIND_BUTT_X,y1=FIND_BUTT_Y;
x2=FIND_BUTT_X+SHORT_BUTT_WIDE,y2=IO_TEST_BUTT_Y+SHORT_BUTT_HIGH;
GLCD_SetWindow_Fill(x1,y1,x2,y2,back_color,back_color);  //short cut
x1=PARAMETER_BUTT_X,y1=FIND_BUTT_Y;
x2=IO_TEST_BUTT_X+SHORT_BUTT_WIDE,y2=IO_TEST_BUTT_Y+SHORT_BUTT_HIGH;
GLCD_SetWindow_Fill(x1,y1,x2,y2,back_color,back_color);  //short cut
//--------------，进退.......
x1=BACK_PAGE_X,y1=BACK_PAGE_Y;
...
switch(mode)
	...
	case MANUAL_MODE:	//手动模式..............
			...
			// else
				ElectricalWheel_dis();  //F4电子手轮  开关状态
...
switch(mode)
	...
	case MANUAL_MODE:	//手动模式..............
			...
			ManualIconDown_dis(MANUAL_BUTT_X, MANUAL_BUTT_Y);//手动----------------------
			AutoIconUp_dis(AUTO_BUTT_X,AUTO_BUTT_Y);//自动up
			TeachModeUp_dis(TEACH_BUTT_X ,TEACH_BUTT_Y );//示教按钮up
			AutoCutMOdeUp_dis(AUTOCUT_BUTT_X ,AUTOCUT_BUTT_Y );//自刀按钮up
			break;
	case AUTO_MODE://自动模式..............
			DatasTableFunPic_dis( TABLE_FUN_PIC_X,  TABLE_FUN_PIC_Y);//列表模式pic
			ManualIconUp_dis(MANUAL_BUTT_X, MANUAL_BUTT_Y);//手动up
			AutoIconDown_dis(AUTO_BUTT_X,AUTO_BUTT_Y);//自动-------------
			TeachModeUp_dis(TEACH_BUTT_X ,TEACH_BUTT_Y );//示教按钮up
			AutoCutMOdeUp_dis(AUTOCUT_BUTT_X ,AUTOCUT_BUTT_Y );//自刀按钮up
			break;
	// case READ_PRO_MODE:		//查找模式..............
	...
	case SELF_CUT_MODE:  //自刀主 模式
			ManualIconUp_dis(MANUAL_BUTT_X, MANUAL_BUTT_Y);//手动up
			AutoIconUp_dis(AUTO_BUTT_X,AUTO_BUTT_Y);//自动
			TeachModeUp_dis(TEACH_BUTT_X ,TEACH_BUTT_Y );//示教按钮up
			AutoCutMOdeDown_dis(AUTOCUT_BUTT_X ,AUTOCUT_BUTT_Y );//自刀按钮-------------
			break;
	case TEACH_MODE://示教 模式
			ManualIconUp_dis(MANUAL_BUTT_X, MANUAL_BUTT_Y);//手动up
			AutoIconUp_dis(AUTO_BUTT_X,AUTO_BUTT_Y);//自动up
			TeachModeDown_dis(TEACH_BUTT_X ,TEACH_BUTT_Y );//示教按钮------------------
			AutoCutMOdeUp_dis(AUTOCUT_BUTT_X ,AUTOCUT_BUTT_Y );//自刀按钮up
			break;
	}
...
switch(Language)
	{
	case CHN_HAN:
		Printf24("序号        名称",x,y,line_color,0,back_color);	x+=360;
		Printf24("步骤      修改日期",x,y,line_color,0,back_color);
			break;
	case ENGLISH:	
		Printf24("Order       Name",x,y,line_color,0,back_color);	x+=360;
		Printf24("Step      Modified",x,y,line_color,0,back_color);
			break;
	case INDONSIA:	//印尼	
		Printf16("Tidak.      Nama",x,y,line_color,0,back_color);	x+=400;
	...
	case 3:
		...
		if(flag==1)
			...
			switch(Language)
				{
				case CHN_HAN:
					Printf24("按<>,切换语言!           ",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
					break;
				case ENGLISH:	
					Printf24("Press <>,change language!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
					break;
				case INDONSIA:	//印尼
					Printf16("Tekan tombol <> untuk beralih bahasa",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;	
				case PORTUGAL:	//葡萄牙
					Printf16("Pressione a tecla <> para alternar idiomas",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;	
				case SPANISH: //西班牙
					Printf16("Pulse la tecla <> para interruptor",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;	
				}	
			}	
		break;
	case 4:
		...
		if(flag==1)
			...
			switch(Language)
				{
				case CHN_HAN:
					Printf24("按<>,切换公英制!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
					break;
				case ENGLISH:	
					Printf24("Press <>,change unit!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
					break;
				case INDONSIA:	//印尼
					Printf16("Tekan tombol <> untuk beralih unit",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;	
				case PORTUGAL:	//葡萄牙
					Printf16("Pressione a tecla <> para comutar a unidade",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;	
				case SPANISH: //西班牙	
					Printf16("Pulse la tecla <> para cambiar la unidad",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;		
				}
			}	
		break;
			...
			GLCD_SetWindow_Fill( x, y, x+tx_wide, y+24, TEXT_TAB_BACK_COLOR, TEXT_TAB_BACK_COLOR);//clear text win
				...
				if(flag==1)	
					...
					switch(Language)
						{
						case CHN_HAN:
							Printf24("按<>,切换光电有效电平!     ",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
							break;
						case ENGLISH:	
							Printf24("Press <>,change IR status!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
							break;
						case INDONSIA:	//印尼	
							Printf16("Tekan tombol <>,Switching Status IR",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
							break;
						case PORTUGAL:	//葡萄牙
							Printf16("Pressione a tecla <> para ligar o sinal IR",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
							break;
						case SPANISH: //西班牙
							Printf16("Pulse la tecla <> para cambiar IR",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
							break;
						}
					}
					break;
	...
	case 7:  //压纸有效 标志
		...
		if(flag==1)	
			...
			switch(Language)
				{
				case CHN_HAN:
					Printf24("按<>,切换压纸停机功能!     ",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
					break;
				case ENGLISH:	
					Printf24("Press <>,change Press-Stop!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
					break;
				case INDONSIA:	//印尼	
					Printf16("Tekan tombol <>,Platen berhenti",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
					break;
				case PORTUGAL:	//葡萄牙
					Printf16("Pressione a tecla <>, a fun??o comuta??o",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
					...
					break;
				case SPANISH: //西班牙
					Printf16("Pulse la tecla <> cambiar stop",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
					break;
				}
			}
		break;	
		...
		switch(MotorType)
			...
			case MOTOR_AC_A6_SERVO:
				...
				if(flag==1)
					...
					Printf24("0        8",REMIND_WINDOW_X+200,REMIND_WINDOW_Y+5,RED_COLOR,0,0xffff);	
					}
					break;
			}		
		break;
			...
			case MOTOR_AC_A6_SERVO:
				...
				if(LuoJu_set==0)
					...
					switch(Language)
						...
						case PORTUGAL:	//葡萄牙
						case SPANISH: //西班牙
							Printf24(" 10mm ",x,y,front_color,1,back_color); //
								break;
						}		
					}
				else
					...
					switch(Language)
						...
						case PORTUGAL:	//葡萄牙
						case SPANISH: //西班牙
							Printf24(" 20mm ",x,y,front_color,1,back_color); //
								break;
						}		
					}
				if(flag==1)	
					...
					switch(Language)
						{
						case CHN_HAN:
							Printf24("按<>,选择螺距!     ",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
							break;
						case ENGLISH:	
						case INDONSIA:	//印尼	
						case PORTUGAL:	//葡萄牙
						case SPANISH: //西班牙
							Printf24("Press <>,select screw!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
							break;
						}
					}
					break;
...
switch(sel)
	...
	case 1://
			switch(MotorType)
				...
				case MOTOR_FREQ_5_SPD_RST:
				case MOTOR_FREQ_3_SPD:
				case MOTOR_FREQ_3_SPD_RST:  //变频器--误差掩盖值
						if(Unit==UNIT_MM)//mm  单位
				...
				case MOTOR_AC_SERVO:
				case MOTOR_AC_SERVO_REV:
				case MOTOR_AC_A6_SERVO://红外切换工作模式
						GLCD_SetWindow_Fill(x, y, x+tx_wide, y+24, TEXT_TAB_BACK_COLOR, TEXT_TAB_BACK_COLOR);//clear text win
						...
						if(flag==1)
							...
							switch(Language)
								{
								case CHN_HAN:
									Printf24("按<>设置IR切换工作模式",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
									break;
								case ENGLISH:	
								case INDONSIA:	//印尼
								case PORTUGAL:	//葡萄牙
								case SPANISH: //西班牙
									Printf24("press <> change IR",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
										break;
								}	
							}
	...
	case 2:
			switch(MotorType)
					...
					case MOTOR_FREQ_5_SPD_RST:
					case MOTOR_FREQ_3_SPD:
					case MOTOR_FREQ_3_SPD_RST://红外切换工作模式
						GLCD_SetWindow_Fill(x, y, x+tx_wide, y+24, TEXT_TAB_BACK_COLOR, TEXT_TAB_BACK_COLOR);//clear text win
						if(IRChgWorkmodeValid_flag!=0)
							{
							switch(Language)
...
switch(Language)
	...
	case CHN_HAN:
		...
		switch(MotorType)
			...
			case MOTOR_FREQ_3_SPD_RST:
				num=9;
				Printf24("提前量:  ",x,y,BLACK_COLOR,1,back_color);
					break;
			// // case MOTOR_DC_SERVO:
			// // case MOTOR_DC670_SERVO:
			...
			case MOTOR_AC_A6_SERVO:
				...
				Printf24("螺距选择:  ",x,y,BLACK_COLOR,1,back_color);y+=gap;
	...
	case ENGLISH:	
		...
		switch(MotorType)
			...
			case MOTOR_FREQ_3_SPD_RST:
				num=9;
				Printf24("Advanced:",x,y,BLACK_COLOR,1,back_color);
					break;
			// // case MOTOR_DC_SERVO:
			// // case MOTOR_DC670_SERVO:
			...
			case MOTOR_AC_A6_SERVO:
				...
				Printf24("Scr-pitch: ",x,y,BLACK_COLOR,1,back_color);y+=gap;
	...
	case INDONSIA:	//印尼
		...
		Printf24("Satuan:",x,y,BLACK_COLOR,1,back_color);  y+=gap;
				Printf24("Set-IR:  ",x,y,BLACK_COLOR,1,back_color);y+=gap;
		...
		switch(MotorType)
			...
			case MOTOR_FREQ_3_SPD_RST:
				num=9;
				Printf24("Memajukan:",x,y,BLACK_COLOR,1,back_color);
					break;
			// // case MOTOR_DC_SERVO:
			// // case MOTOR_DC670_SERVO:
	...
	case PORTUGAL:	//葡萄牙
		...
		Printf24("antes:",x,y,BLACK_COLOR,1,back_color);y+=gap;
				Printf24("estado IR:",x,y,BLACK_COLOR,1,back_color); y+=gap;
		...
		switch(MotorType)
			...
			case MOTOR_FREQ_3_SPD_RST:
				num=9;
				Printf24("unidade:",x,y,BLACK_COLOR,1,back_color); 
					break;
			// // case MOTOR_DC_SERVO:
			// // case MOTOR_DC670_SERVO:
	...
	case SPANISH: //西班牙
		...
		switch(MotorType)
			...
			case MOTOR_FREQ_3_SPD_RST:
				num=9;
				Printf24("avanzar:",x,y,BLACK_COLOR,1,back_color);
					break;
			// // case MOTOR_DC_SERVO:
			// // case MOTOR_DC670_SERVO:
				// // Printf24("Veloci-má  ",x,y,BLACK_COLOR,1,back_color);
	...
	case CHN_HAN:
		switch(MotorType)
				...
				case MOTOR_FREQ_3_SPD_RST:
					...
					Printf24("IR切换模式:    ",x,y,BLACK_COLOR,1,back_color);y+=gap;
						break;
...
if(flag==1)
	...
	switch(Language)
		{
		case CHN_HAN:
				Printf24("再按++--,退出",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
		case ENGLISH:	
				Printf24("Press ++-- to exit",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
		case INDONSIA:	//印尼	
				Printf24("Tekan ++-- untuk keluar",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
		case PORTUGAL:	//葡萄牙
				Printf24("imprensa ++--,desistir",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
		case SPANISH: //西班牙
				Printf24("prensa ++--,dejar de",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
		}
	//.........................解释 框
...
BackWindNote_dis(x , y);x+=10,y+=20;//note window
...
switch(Language)
	{
	case CHN_HAN:
		Printf24("自刀模式高度危险，请",x,y,front_color,0,back_color);y+=30;
		Printf24("按“确认”键进入！",x,y,front_color,0,back_color);x+=40,y+=50;
		Printf24("确认",x,y,front_color,0,back_color);x+=150;
		Printf24("取消",x,y,front_color,0,back_color);
			break;
	case ENGLISH:	
		Printf24("Auto-Cut mode is so",x,y,front_color,0,back_color);y+=30;
		Printf24("dangerous, press ENTER!",x,y,front_color,0,back_color);x+=40,y+=50;
		Printf24("OK",x,y,front_color,0,back_color);x+=120;
		Printf24("Cancel",x,y,front_color,0,back_color);
			break;
	case INDONSIA:	//印尼
		Printf24("Modus pemotongan otomatis",x,y,front_color,0,back_color);y+=30;
		Printf24("sangat berbahaya!",x,y,front_color,0,back_color);x+=20,y+=50;
		Printf24("Ya",x,y,front_color,0,back_color);x+=130;
		Printf24("Membatal",x,y,front_color,0,back_color);
			break;
	case PORTUGAL:	//葡萄牙
		Printf16("Este modo é muito perigoso",x,y,front_color,0,back_color);y+=30;
		Printf16("Pressione o bot?o OK para entrar",x,y,front_color,0,back_color);
		ASCII_Printf("a",ASCII16_MODE,8,16,PUTAO_ZK16, x+8*15,y,front_color,back_color);x+=20,y+=50;
		Printf24("OK",x,y,front_color,0,back_color);x+=130;
		Printf24("Cancelar",x,y,front_color,0,back_color);
			break;
	case SPANISH: //西班牙
		Printf16("Este modo es muy peligroso",x,y,front_color,0,back_color);y+=30;
		Printf16("Pulse el botón OK para entrar",x,y,front_color,0,back_color);x+=20,y+=50;
		Printf24("OK",x,y,front_color,0,back_color);x+=130;
		Printf24("Cancelar",x,y,front_color,0,back_color);
			break;
	}
...
switch(Language)
	{
	case CHN_HAN:
		Printf24("是否继续执行之前的",x,y,front_color,0,back_color);y+=30;
		Printf24("自刀步骤？",x,y,front_color,0,back_color);x+=40,y+=50;
		Printf24("确认",x,y,front_color,0,back_color);x+=150;
		Printf24("取消",x,y,front_color,0,back_color);
			break;
	case ENGLISH:	
		Printf24("Continue the last",x,y,front_color,0,back_color);y+=30;
		Printf24("Auto-Cut step?",x,y,front_color,0,back_color);x+=40,y+=50;
		Printf24("OK",x,y,front_color,0,back_color);x+=120;
		Printf24("Cancel",x,y,front_color,0,back_color);
			break;
	case INDONSIA:	//印尼
		Printf24("Apakah akan melanjutkan ",x,y,front_color,0,back_color);y+=30;
		Printf24("langkah-langkah sebelumnya?",x,y,front_color,0,back_color);x+=40,y+=50;
		Printf24("Ya",x,y,front_color,0,back_color);x+=130;
		Printf24("Membatal",x,y,front_color,0,back_color);
			break;
	case PORTUGAL:	//葡萄牙
		Printf24("Se continuar a opera??o",x,y,front_color,0,back_color);
		ASCII_Printf("ca",ASCII24_MODE,12,24,PUTAO_ZK24, x+12*20,y,front_color,back_color);y+=30;
		Printf24("anterior?",x,y,front_color,0,back_color);x+=20,y+=50;
		Printf24("OK",x,y,front_color,0,back_color);x+=130;
		Printf24("Cancelar",x,y,front_color,0,back_color);
			break;
	case SPANISH: //西班牙
		Printf24("Ya sea para continuar",x,y,front_color,0,back_color);//Ya sea para continuar la ejecución
		y+=30;
		Printf24("la ejecució",x,y,front_color,0,back_color);//ejecución
	...
	case 0://第一次 天数		
		...
		if(flag==1)
			...
			switch(Language)
				{
				case CHN_HAN:
					Printf24("输入天数,按<>生效!",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
				case ENGLISH:	
					Printf24("Enter days,press <>",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
				case INDONSIA:	//印尼
					Printf24("Input Data,Tekan tombol <>",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
				case PORTUGAL:	//葡萄牙
					Printf16("Os dados de entrada, pressione a tecla <>",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
				case SPANISH: //西班牙
					...
					Printf16("                      as, pulse la tecla <>",REMIND_WINDOW_X+10,REMIND_WINDOW_Y+5,BLACK_COLOR,0,0xffff);
						break;
				}
			}
		x+=36;
...
TeachModeUp_dis( TEACH_BUTT_X, TEACH_BUTT_Y);//示教按钮
AutoCutMOdeUp_dis( AUTOCUT_BUTT_X, AUTOCUT_BUTT_Y);//自刀按钮
...
//****---****--**--
//GLCD_SetWindow(500,CALCUL_WIN_Y,500+450,CALCUL_WIN_Y+330,RED_COLOR);
}
...
if(ModBus_AcErr_Type==AC_ERR_Z)	//Z 信号故障		//警报 不显示尺寸
	{
	ASCII_Printf(Err_XXX,192+1,32,48,ASCII_32x48, x,y,RED_COLOR,back_color);//	20171219
...
Int8U num[4],k;
back_color=STEP_BACK_COLOR;//----------
x=STEP_NUM_X+12*4,y=STEP_NUM_Y;
...
switch(motor)
	...
	case MOTOR_AC_SERVO:	 		//交流伺服	
		...
		switch(Language)
			{
			case CHN_HAN:
				Printf24("1------交流伺服正向",x,y,F_color,1,B_color);
				break;
			case ENGLISH:
			case INDONSIA:	//印尼
			case PORTUGAL:	//葡萄牙
			case SPANISH: //西班牙
				Printf24("4------AC servo Forward",x,y,F_color,1,B_color);
				break;
			}
		break;	
	case MOTOR_AC_SERVO_REV:	 		//交流伺服	
		...
		switch(Language)
			{
			case CHN_HAN:
				Printf24("2------交流伺服反向",x,y,F_color,1,B_color);
				break;
			case ENGLISH:
			case INDONSIA:	//印尼
			case PORTUGAL:	//葡萄牙
			case SPANISH: //西班牙
				Printf24("5------AC servo Reverse",x,y,F_color,1,B_color);
				break;
			}
		break;	
	case MOTOR_AC_A6_SERVO:	 		//交流伺服A6	
		y+=gap*5;
		switch(Language)
			{
			case CHN_HAN:
				Printf24("6------交流伺服A6",x,y,F_color,1,B_color);
				break;
			case ENGLISH:
			case INDONSIA:	//印尼
			case PORTUGAL:	//葡萄牙
			case SPANISH: //西班牙
				Printf24("6------AC servo (A6)",x,y,F_color,1,B_color);
				break;
			}
		break;	
	}
...
Num[i]=0;
x=CALCUL_TEXT_END-(16*i),y=CALCUL_TEXT_Y+5;
...
Num[i]=0;
x=CALCUL_TEXT_END-(16*i),y=CALCUL_RESULT_Y+5;
Printf32(Num,x,y,num_color,1,0xffff);	
}
...
void FlashSignButton(void)
{
if((WorkMode==AUTO_BATCH_MODE)||(WorkMode==TEACH_BATCH_MODE))  //F5++--
	{
	flashbutt_time++;
	if(flashbutt_time>=500)
		{
		flashbutt_time=0;	
		switch(flashbutt_mode)
			{
			case 0:
				Button_dis(BUTT_PLUS, 0); //触摸 按钮 
				Button_dis(BUTT_SUB, 1); //触摸 按钮 
				flashbutt_mode++;
					break;
			case 1:
				Button_dis(BUTT_PLUS, 1); //触摸 按钮 
				Button_dis(BUTT_SUB, 0); //触摸 按钮 
				flashbutt_mode=0;
					break;
			case 2:	
				flashbutt_mode=0;
