...
if(WorkStart_flag==0)  //未进入工作状态
	...
	if(Touch_code==0)
		...
		if(WorkMode==MANUAL_MODE)   //手动模式 前进，后退按钮
			{
			...
			else if((Touch_Pos.y>=MAN_FAST_FWD_Y1)&&(Touch_Pos.y<=MAN_FAST_FWD_Y2))
				{
			...
			if(Touch_code==0)
				...
				switch(WorkMode)
					...
					case SELF_MINUS_MODE://自刀 F4--
						...
						if((WorkMode==AUTO_MODE)||(WorkMode==PROG_MODE))
							{
							...
							if(WorkMode==AUTO_MODE)
								{
								if((Touch_Pos.y>=F5PRE_FLG_Y1)&&(Touch_Pos.y<=F5PRE_FLG_Y2))
									{
									if((Touch_Pos.x>=F5PRE_FLG_X1)&&(Touch_Pos.x<=F5PRE_FLG_X2))
										Touch_code=F5PRE_FLG_TOUCH_CODE;//F5 压纸循环flag
									}	
								}
	...
	if((Touch_code&0xffff0000)==0xffff0000)  //纯 触摸键(特殊)-----------------------------则转为键盘对应码
		...
		switch(Touch_code)
			...
			case F5CUT_TOUCH_CODE:  //F5 1个刀标志 按钮
					break;
			case F5PRE_FLG_TOUCH_CODE:	//F5 压纸循环flag
