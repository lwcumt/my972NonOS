...
for(block=42;block<NF_BLOCKS;block++)  //后续块未使用
	{
...
unsigned char FileRead(unsigned char *fname,unsigned char *sdbuf)//读取文件到sdbuf
{
...
if(File_Find((unsigned char *)&nametemp)==1)
	{//已存在更新
		...
		for(i=0;i<sector;i++)
			{
			SectorInUser[i+iFilesTemp.FileAdd_Sector]=UNUSER;  //清除块占用
			}
		...
		filenumtemp=FileNum;
	//	iFiles.FileAdd_Sector=File_GetFreeSector_New();//iFilesTemp.FileAdd_Sector;//获取可用块
	...
	iFiles.FileLength=length;
	iFiles.FileAdd_Sector=File_GetFreeSector_New();//File_GetFreeSector();//获取可用块
	if(iFiles.FileAdd_Sector>=42)
		{
