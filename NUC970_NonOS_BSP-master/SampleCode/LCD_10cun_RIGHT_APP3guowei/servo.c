...
if(ServoRecvOk_flag==1)
	...
	if(ServoDisconnect_flag==1)
		{
		CurrentArea_recover2(START_PROMPT_X2, START_PROMPT_Y2,    START_PROM_WIDE, START_PROM_HIGH); //清 驱动器断开   提示
...
if(ServoConnect_flag==1)
	...
	if(ServoDisconnect_flag==0)
		...
		switch(Language)
			{
			case CHN_HAN:
					Printf24("驱动器未连接!",START_PROMPT_X2,START_PROMPT_Y2+5,RED_COLOR,1,0xffff);
					break;
			case ENGLISH:
					Printf24("The driver isn't connected!",START_PROMPT_X2,START_PROMPT_Y2+5,RED_COLOR,1,0xffff);
					break;
			case INDONSIA:	//印尼
					Printf24("Drive tidak terhubung!",START_PROMPT_X2,START_PROMPT_Y2+5,RED_COLOR,1,0xffff);
					break;
			case PORTUGAL:	//葡萄牙
					Printf24("A unidade n?o estáconectado",START_PROMPT_X2,START_PROMPT_Y2+5,RED_COLOR,1,0xffff);
					ASCII_Printf("a",ASCII24_MODE,12,24,PUTAO_ZK24,START_PROMPT_X2+12*11,START_PROMPT_Y2+5,RED_COLOR,0xffff);
					break;
			}
