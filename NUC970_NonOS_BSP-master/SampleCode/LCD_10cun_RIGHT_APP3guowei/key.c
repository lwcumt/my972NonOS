...
if(WorkStart_flag==1)
	...
	if(Run_flag==0)
		{
		if(ButtonTouch_mode==TOU_MODE_F4) // 标记框模式
			{
			if(num_cou<7)
				{
				if(num!='.')
					...
					if(SubLine_flag==0)
						...
						if((num=='-')&&(num_cou>0))
							...
							SubLine_flag=1;
							//-----计算‘-’之前数值
							for(c_t=0;c_t<num_cou;c_t++)
								{
								temp=temp*10+Size_temp[c_t];
								}
							FirstKnife_num=temp;
							LastKnife_num_temp=0;
							}
						Size_temp[num_cou++]=num;
						InputNum_dis(Size_temp,num_cou);//*********显示
						}
					else	//  已输入 ‘-’
						{
						if(num!='-')
							...
							Size_temp[num_cou++]=num;
							InputNum_dis(Size_temp,num_cou);//*********显示
							LastKnife_num_temp=LastKnife_num_temp*10+num;
							}
						}
					}
				}
				...
				default:
					...
					if(num_cou<7)
						...
						if(num!='.')
							...
							else if(ButtonTouch_mode==TOU_MODE_CALC)//--------------------计算器
								{
...
if(WorkMode==MANUAL_MODE)  // 手动模式	默认当前尺寸
	{
	if(Unit==UNIT_MM)//mm
		{
...
switch (keycode)
	 ...
	 case 1:      //XXXXXXXXXXXX
			...
			if(WorkStart_flag==0)  //未进入工作状态
				...
				if(CheckModeOK_flag==1)
					...
					if(Board_TestMod==0)
						{
						Board_TestMod=TEST_SYS_TYPE;     //电机类型设定  模式
	...
	case DENGJU_KEY_CODE+NINE_KEY_CODE:			//向上箭头+1  脉冲当量 设定
			...
			if(WorkStart_flag==1)
			  {
			   if((MotorType==MOTOR_DC_SERVO)||(MotorType==MOTOR_DC670_SERVO))    //DC伺服--
				  {
				  if(Run_flag==0)
					 {
					 if(ButtonTouch_mode==0)
...
switch(keycode)
	...
	case DEL_ALL_KEY_CODE:	//全删--
			if(ButtonTouch_mode==0)//未有  弹出页
				...
				switch(WorkMode)
					...
					case PROG_MODE:
					case TEACH_MODE:
					//case AUTO_MODE://编程,示教,自动画面下---------------------全删 所有刀数据
	...
	case SHIFT_KEY_CODE:	//shift--
			if(ShiftKey_flag==1)
				...
				break;
				/**
	case SHIFT_F5_KEY_CODE:	//shift+F5------ 添加所有刀标记
			if(WorkStart_flag==1)
			  {
			  if(Run_flag==0)
				 {
				 if(ButtonTouch_mode==TOU_MODE_F4)		//F4 标记模式
					{
			  ...
			  break;***/
	case SHIFT_0_KEY_CODE:	//shift+0------ 清除所有刀标记
			if(WorkStart_flag==1)
			  {
			  if(Run_flag==0)
				 {
					...
					switch(WorkMode)
						...
						case TEACH_MODE://示教 主画面下
							if(AllKnifeSet_flag== 0)
								...
								AllKnifeSet_flag=0;
							AllKnifeFlagExec(AllKnifeSet_flag);
								break;
						}
					}
				 }	
	...
	case SHIFT_8_KEY_CODE:	//shift+8------ 密码 模式
			if(WorkStart_flag==1)
			  ...
			  if(Run_flag==0)
				...
				break;
	case SHIFT_1_KEY_CODE:	//shift+1------直流伺服 C 参数
			if(WorkStart_flag==1)
			  {
			   if((MotorType==MOTOR_DC_SERVO)||(MotorType==MOTOR_DC670_SERVO))    //DC伺服--
				  {
						...
						case SELF_TOOLS_MODE:
							...
							//	{
								...
								if((MotorType==MOTOR_DC_SERVO)||(MotorType==MOTOR_DC670_SERVO))
									...
									ParameterPage_dis(10, 0);
									// ParameterPage_dis(11, 0);
									}
			...
			if(Run_flag==0)
				...
				if(ButtonTouch_mode==0)
					...
					switch(WorkMode)
						...
						//case PROG_PRO_MODE://已在  程序选择画面	xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
							cursor_pro=0;		
							WorkMode=PROG_MODE;//------------------退出返回到编程主 画面
							PorgSet_sel=0;
							CurrentArea_recover2(MIDDLE_PAGE_X , MIDDLE_PAGE_Y,  M_PAGE_WIDE, M_PAGE_HIGH);
						...
						//case TEACH_DIVIDE_MODE://已在 等分画面 		
						//case TEACH_PRO_MODE://已在  程序选择画面	xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
							cursor_pro=0;		
							WorkMode=TEACH_MODE;//------------------退出返回到示教主 画面
							PaperCutEnb_signal=1;
							PorgSet_sel=0;
		...
		if(WorkStart_flag==1)
		   {
			if(Run_flag==0)
				...
				if(ButtonTouch_mode==0)
					...
					{
					// AllFlagSetExec(CRASH_FLAG); //设定 撞纸标记
					}
			    }
			else  //运行中
			    {
			...
			switch(MotorType)
				...
				case MOTOR_AC_SERVO_REV:
					if(RunEnb_flag==1)
						...
						if(Board_TestMod==0)       //-----
							{
							if((StartCheck_mode==2)||(StartCheck_mode==3))    //-------------2: 后极限ok 进入---------3: only后极限异常进入
								{
								if(PassCodeEn_flag==0)
									{
									CurrentArea_recover2(START_PROMPT_X2, START_PROMPT_Y2, START_PROM_WIDE, START_PROM_HIGH);
									...
									//////////////
									if(StartCheck_mode==2) //-------------2: 后极限ok 进入
										{
										StartCheck_mode=7;
						...
						if(num_cou>0)
							...
							if(Knife_order<=OnePro_head.pro_knifeSUM)
							   ...
							   if(CheckInputNums()==1)
									...
									if(ButtonTouch_mode==0)  //	 非 触摸计算器 下
										ClrInput_function();//清零 输入框
									else if(ButtonTouch_mode==TOU_MODE_CALC)	//触摸 计算器下
										CalcuInClear_exec();     //清除 计算器
									WorkMode=AUTO_MODE;
									PaperCutEnb_signal=1;
									F3_plus_dis(0);	
						...
						if(num_cou>0)
							...
							if(Knife_order<=OnePro_head.pro_knifeSUM)
							   ...
							   if(CheckInputNums()==1)
									...
									if(ButtonTouch_mode==0)  //	 非 触摸计算器 下
										ClrInput_function();//清零 输入框
									else if(ButtonTouch_mode==TOU_MODE_CALC)	//触摸 下
										CalcuInClear_exec();     //清除 计算器	
									WorkMode=SELF_CUT_MODE; 
									F3_plus_dis(0);	
									}
		  ...
		  if(Board_TestMod==TEST_NO)
			...
			switch(MotorType)
				...
				case MOTOR_FREQ_3_SPD_RST:
					 ...
					 else if((StartCheck_mode==4)||(StartCheck_mode==5))//else //if(StartCheck_mode==4)
						 {
				...
				case MOTOR_AC_SERVO:
				case MOTOR_AC_SERVO_REV:
					if((StartCheck_mode==2)||(StartCheck_mode==3))
						 {
						 if(PassCheckDis_flag==1) //开机 密码下
							 {
							 if(num_cou>0)
...
x2=F1LABEL_BUTT_X-10; y2=y1+PROTEXT_HIGH*PROGS_1PAGE-10;
...
nn=0;
Dot_flag=0;
if((ButtonTouch_mode==0)||(ButtonTouch_mode==TOU_MODE_F4)) // 标记框模式
	{
	Size=Size_temp;
	num_bits=num_cou;
	}
else if(ButtonTouch_mode==TOU_MODE_CALC)    //触摸 输入下
	{
				...
				if(CheckInputNums()==1)//数据有效
					...
					if(Total_size!=InputSize)
						...
						if(Label_size*Waste_size>=0) //数据 已经输入完毕
							{
							if(ButtonTouch_mode==TOU_MODE_CALC)   //触摸下  计算器 弹出框消失
								{
								CalcuInClear_exec();     //清除 计算器
								}
							}
...
KnifeUp_dis(KNIFE_UP_X,KNIFE_Y);//刀上
PressUp_dis(PRESS_UP_X,PRESS_UP_Y);//压上
		...
		if(ButtonTouch_mode==0)//未有  弹出页
			...
			switch(WorkMode)
				...
				case AUTO_MODE:
						...
						/**	
				case PROG_DEL_ALL_PRO_MODE:
				case TEACH_DEL_ALL_PRO_MODE:	//全删 提示模式
						DeleteAllProg();		//F5 删除所有程序
						DeleteAllExitWind();	//退出全删 提示框
							break;**/
				}
