...
VirtualTargetSizeGap=MARGIN_SIZE;
while(1)
   {
   //ACMotor_GetZpulse();
...
if((WorkMode==SELF_CUT_MODE)||(WorkMode==AUTO_MODE)||(WorkMode==TEACH_MODE))  //自刀,自动 示教主模式
	...
	if((MotorType>MOTOR_FREQ_5_SPD_RST)&&(MotorType<MOTOR_DC_SERVO)||(MotorType>=MOTOR_AC_SERVO))	//气垫标志  则到位断开气阀
		{
		
		if(AirOffStart_flag==1)	//气阀已断开
			{
			AirOffStart_flag=0;
		...
		if(MiddleLimit_IN!=0)
			...
			if(outLimM_time>=20)
				...
				if(WorkStart_flag==1)
					...
					{
					x=START_PROMPT_X2,y=START_PROMPT_Y2;
					GLCD_SetWindow_Fill( x, y, x-1+600, y-1+30, 0xfffff, 0xffff);//提示
					}
				switch(Language)
					{
					case CHN_HAN:
						...
						if(OutBack_limit==0)
							...
							if(PushRstKey_flag==0)
								...
								if(RunDisplay_flag==0)
									{
									RunDisplay_flag=1;
									GLCD_SetWindow_Fill( START_PROMPT_X2, START_PROMPT_Y2, START_PROMPT_X2-1+600, START_PROMPT_Y2-1+30, 0xfffff, 0xffff);//提示框
									switch(Language)
										{
										case CHN_HAN:
												Printf24("电脑检测完成,请按运行键!",START_PROMPT_X2,START_PROMPT_Y2+5,RED_COLOR,1,0xffff);
													break;
										case ENGLISH:
			...
			/*
				...
				if(MotorType>MOTOR_FREQ_5_SPD_RST)
					...
					if(AirOffDistance>0)	//关气阀距离有效
						{
						if(AirOffStart_flag==0)	//气阀断开 未启动
							{
...
switch(StartCheck_mode)
	...
	case 5:			
			if(OutFront_limit==1)//前极限
				...
				break;
	case 6:			//前进一段距离， 后极限	恢复正常后----------	开始  后退正常开机步骤
			stop_time++;
			if(stop_time>=1000)
				{
				stop_time=0;
	...
	case 8://-----	 低速后退
			...
			break;
	case 9:	//----delay---- 增加 后限位&原点 判断-----------------20160123
			if(stop_time<2000)
				{
				stop_time++;
				if(senor==0&&status==8)   //前进
